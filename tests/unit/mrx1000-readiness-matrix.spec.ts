 ...
 *   9. The 267 ledger-side title UUID planning handles are tracked separately
...
describe('MRX1000 readiness matrix', () => {
  ...
  it('tracks 267 ledger title UUID planning handles separately from authoritative evidence', () => {
    const ledger = JSON.parse(readFileSync(LEDGER, 'utf8'));
    ...
    expect(ledgerTitleUuids.size).toBe(267);
    expect(matrix.aggregate.ledger_searchatlas_title_uuid_planning_handle_count).toBe(267);
    // Authoritative evidence still requires the title UUID to actually appear
    // in a current readonly artifact. None of the 267 ledger UUIDs match any
...
describe('MRX1000 readiness matrix — load-bearing fact regressions', () => {
  ...
  it('267 topical-map planning handles report map IDs and do not claim article-created proof', () => {
    const evidence = matrix.searchatlas_evidence.topical_map_planning_handles;
    expect(evidence.ledger_searchatlas_title_uuid_planning_handle_count).toBe(267);
