 ...
 *   9. The 205 ledger-side title UUID planning handles are tracked separately
...
describe('MRX1000 readiness matrix', () => {
  ...
  it('tracks 205 ledger title UUID planning handles separately from authoritative evidence', () => {
    const ledger = JSON.parse(readFileSync(LEDGER, 'utf8'));
    ...
    expect(ledgerTitleUuids.size).toBe(205);
    expect(matrix.aggregate.ledger_searchatlas_title_uuid_planning_handle_count).toBe(205);
    // Authoritative evidence still requires the title UUID to actually appear
    // in a current readonly artifact. None of the 205 ledger UUIDs match any
  ...
  it('separates Content Genius exact-title records from ledger-created UUID evidence', () => {
    ...
    expect(matrix.aggregate.content_genius_exact_title_match_rows).toBe(150);
    expect(matrix.aggregate.content_genius_exact_title_unambiguous_rows).toBe(144);
    expect(matrix.aggregate.content_genius_exact_title_ambiguous_rows).toBe(6);
    expect(matrix.aggregate.content_genius_exact_title_total_records).toBe(157);
    expect(matrix.aggregate.ledger_content_genius_article_uuid_count).toBe(0);
...
describe('MRX1000 readiness matrix — load-bearing fact regressions', () => {
  ...
  it('205 topical-map planning handles report map IDs and do not claim article-created proof', () => {
    const evidence = matrix.searchatlas_evidence.topical_map_planning_handles;
    expect(evidence.ledger_searchatlas_title_uuid_planning_handle_count).toBe(205);
