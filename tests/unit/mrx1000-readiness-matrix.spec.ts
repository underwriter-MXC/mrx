 ...
 *  12. The 9 legacy rows plus 40 quality-cleared release rows are preserved as
...
describe('MRX1000 readiness matrix', () => {
  ...
  it('tracks 269 ledger title UUID planning handles separately from authoritative evidence', () => {
    const ledger = JSON.parse(readFileSync(LEDGER, 'utf8'));
    ...
    expect(ledgerTitleUuids.size).toBe(269);
    expect(matrix.aggregate.ledger_searchatlas_title_uuid_planning_handle_count).toBe(269);
    // Authoritative evidence still requires the title UUID to actually appear
    // in a current readonly artifact. None of the 269 ledger UUIDs match any
  ...
  it('preserves the 9 legacy and 40 quality-cleared routes as public_live_known_route', () => {
    const liveRoutes = matrix.rows.filter((r) => r.public_live_known_route);
    expect(liveRoutes.length).toBe(49);
  ...
  it('parses the canonical article sitemap only for current article inclusion', () => {
    ...
    expect(matrix.aggregate.sitemap_currently_included).toBe(49);
  });
...
describe('MRX1000 readiness matrix — load-bearing fact regressions', () => {
  ...
  it('9 legacy and 40 quality-cleared release rows are eligible in the current local sitemap', () => {
  ...
  it('sitemap inclusion is parsed from one selected adapter output and matches the 49 local live rows', () => {
    const selected = selectCanonicalArticlesSitemap(CANONICAL_ARTICLES_SITEMAP_CANDIDATES);
    expect(selected).not.toBeNull();
    expect(matrix.inputs.sitemap_xml_files_indexed).toEqual([
      path.relative(MRX_ROOT, selected as string),
  ...
  it('public article-route counts are independent of release authorization', () => {
    ...
    expect(matrix.aggregate.public_live_known_route).toBe(49);
    expect(matrix.aggregate.release_authorized).toBe(1000);
    expect(matrix.aggregate.index_authorized).toBe(1000);
    for (const row of live) {
      expect(row.release_index.release_authorized).toBe(true);
  ...
  it('269 topical-map planning handles report map IDs and do not claim article-created proof', () => {
    const evidence = matrix.searchatlas_evidence.topical_map_planning_handles;
    expect(evidence.ledger_searchatlas_title_uuid_planning_handle_count).toBe(269);
