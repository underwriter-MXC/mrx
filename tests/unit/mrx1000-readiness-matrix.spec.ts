...
describe('MRX1000 readiness matrix', () => {
  ...
  it('tracks 227 ledger title UUID planning handles separately from authoritative evidence', () => {
    ...
    expect(ledgerTitleUuids.size).toBe(226);
    expect(matrix.aggregate.ledger_searchatlas_title_uuid_planning_handle_count).toBe(226);
...
describe('MRX1000 readiness matrix — load-bearing fact regressions', () => {
  ...
  it('226 topical-map planning handles report map IDs and do not claim article-created proof', () => {
    const evidence = matrix.searchatlas_evidence.topical_map_planning_handles;
    expect(evidence.ledger_searchatlas_title_uuid_planning_handle_count).toBe(226);
