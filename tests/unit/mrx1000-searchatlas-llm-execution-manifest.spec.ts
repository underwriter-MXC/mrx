...
describe('MRX1000 SearchAtlas + ordered LLM execution manifest', () => {
  ...
  it('projects the current 59 + 69 + 25 + 847 workspace-state partition', () => {
    ...
    expect(manifest.aggregate.workspace_public_route_configured).toBe(59);
    expect(
      manifest.rows.every(
  ...
  it('distinguishes workspace inventory from a checksummed, readiness-backed LLM candidate', () => {
    expect(manifest.aggregate).toMatchObject({
      ...
      by_review_candidate_state: {
        CHECKSUMMED_EXISTING_PUBLIC_ARTICLE_READY_FOR_LLM_REVIEW: 59,
        CHECKSUMMED_HELD_SUBSTANTIVE_DRAFT_READY_FOR_LLM_REVIEW: 69,
