...
describe('MRX1000 SearchAtlas + ordered LLM execution manifest', () => {
  ...
  it('projects the current 89 + 39 + 25 + 847 workspace-state partition', () => {
    expect(manifest.aggregate.by_canonical_inventory_state).toEqual({
      incumbent_draft_nonpublic_held: 39,
      live_public_published_route: 89,
      pilot_draft_noindex_stage: 25,
      planning_only_inventory: 847,
    });
    expect(manifest.aggregate.repo_mdx_present).toBe(153);
    expect(manifest.aggregate.workspace_public_route_configured).toBe(89);
    expect(
      manifest.rows.every(
  ...
  it('distinguishes workspace inventory from a checksummed, readiness-backed LLM candidate', () => {
    expect(manifest.aggregate).toMatchObject({
      ...
      by_review_candidate_state: {
        CHECKSUMMED_EXISTING_PUBLIC_ARTICLE_READY_FOR_LLM_REVIEW: 89,
        CHECKSUMMED_HELD_SUBSTANTIVE_DRAFT_READY_FOR_LLM_REVIEW: 39,
