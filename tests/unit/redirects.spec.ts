import { readFileSync } from 'node:fs';
import { describe, expect, it } from 'vitest';

const vercel = JSON.parse(readFileSync(new URL('../../vercel.json', import.meta.url), 'utf8'));

describe('canonical owner-situation redirects', () => {
  it('permanently redirects the legacy WordPress date archives to canonical article URLs', () => {
    expect(vercel.redirects).toEqual(
      expect.arrayContaining([
        {
          source: '/2026/06/02/:slug',
          destination: '/blog/:slug/',
          permanent: true,
        },
        {
          source: '/2026/06/02/:slug/',
          destination: '/blog/:slug/',
          permanent: true,
        },
        {
          source: '/2026/06/03/:slug',
          destination: '/blog/:slug/',
          permanent: true,
        },
        {
          source: '/2026/06/03/:slug/',
          destination: '/blog/:slug/',
          permanent: true,
        },
      ]),
    );
  });

  it('permanently redirects the obsolete 1031 route', () => {
    expect(vercel.redirects).toContainEqual({
      source: '/1031-exchanger',
      destination: '/1031-exchange/',
      permanent: true,
    });
    expect(vercel.redirects).toContainEqual({
      source: '/1031-exchanger/',
      destination: '/1031-exchange/',
      permanent: true,
    });
  });

  it('permanently redirects the common Contact Us route', () => {
    expect(vercel.redirects).toContainEqual({
      source: '/contact-us',
      destination: '/contact/',
      permanent: true,
    });
  });

  it('consolidates confirmed retired articles into the closest definitive owner-intent pages', () => {
    expect(vercel.redirects).toEqual(
      expect.arrayContaining([
        {
          source: '/blog/1031-exchange-fees-for-mineral-rights-sales/',
          destination:
            '/blog/1031-exchange-for-mineral-rights-does-it-qualify-and-how-does-it-work/',
          permanent: true,
        },
        {
          source: '/blog/factors-that-affect-mineral-rights-value/',
          destination: '/mineral-rights-value/',
          permanent: true,
        },
        {
          source: '/blog/hidden-terms-in-a-mineral-rights-offer/',
          destination: '/blog/how-to-know-if-your-mineral-rights-offer-is-fair/',
          permanent: true,
        },
        {
          source: '/blog/how-a-1031-exchange-benefits-mineral-rights-owners/',
          destination:
            '/blog/1031-exchange-for-mineral-rights-does-it-qualify-and-how-does-it-work/',
          permanent: true,
        },
        {
          source: '/blog/how-mineral-rights-ownership-works/',
          destination:
            '/blog/how-texas-mineral-rights-ownership-works-deeds-conveyances-and-title/',
          permanent: true,
        },
        {
          source: '/blog/how-to-evaluate-mineral-production-in-texas/',
          destination:
            '/blog/texas-oil-and-gas-production-by-county-what-mineral-rights-owners-should-know/',
          permanent: true,
        },
        {
          source: '/blog/how-to-navigate-a-1031-exchange-for-mineral-rights/',
          destination:
            '/blog/1031-exchange-for-mineral-rights-does-it-qualify-and-how-does-it-work/',
          permanent: true,
        },
        {
          source: '/blog/mineral-rights-closing-costs-and-fees/',
          destination: '/blog/closing-costs-and-fees-when-selling-mineral-rights-in-texas/',
          permanent: true,
        },
        {
          source: '/blog/oil-and-gas-royalties-what-owners-need-to-know/',
          destination: '/blog/how-royalty-payments-work-for-texas-mineral-rights-owners/',
          permanent: true,
        },
        {
          source: '/blog/selling-oil-and-gas-rights-step-by-step-guide/',
          destination: '/blog/how-to-sell-mineral-rights-in-texas/',
          permanent: true,
        },
        {
          source: '/blog/tax-on-sale-of-inherited-mineral-rights/',
          destination:
            '/blog/capital-gains-tax-on-mineral-rights-sales-in-texas-what-sellers-need-to-know/',
          permanent: true,
        },
        {
          source: '/blog/understanding-1031-exchange-benefits-for-mineral-rights-owners/',
          destination:
            '/blog/1031-exchange-for-mineral-rights-does-it-qualify-and-how-does-it-work/',
          permanent: true,
        },
        {
          source:
            '/blog/understanding-our-valuation-methodology-ensuring-fairness-and-accuracy-in-your-mineral-rights-review/',
          destination: '/methodology/',
          permanent: true,
        },
        {
          source: '/blog/understanding-probate-mineral-rights-in-texas/',
          destination: '/blog/understanding-the-probate-process-for-mineral-interests/',
          permanent: true,
        },
      ]),
    );
  });
});
