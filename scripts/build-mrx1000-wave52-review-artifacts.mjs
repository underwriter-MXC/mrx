...
const slug = 'understanding-the-steps-to-book-your-free-no-obligation-mineral-rights-review';
const programRowId = 'MRX1000-0132';
const title = 'Understanding the Steps to Book Your Free No-Obligation Mineral Rights Review';
const inlineKeyword = 'What Steps Are Involved in Getting a No-Obligation Review of My Mineral Rights?';
const articlePath = `src/content/posts/${slug}.mdx`;
const creativePath = `artifacts/mrx1000-wave52-creative-qa/${slug}/creative-manifest.json`;
...
if (!row || row.slug !== slug || row.title !== title) {
  throw new Error('Wave 52 batch identity is missing or drifted');
}
...
function sourceScope(source) {
  const scopes = new Map([
    [
      ...
      [
        'The current MRX booking page describes a private scheduling conversation that confirms timezone and timing preference, checks live availability, offers phone times, requests appointment contact details, presents communication choices separately, and confirms the appointment on screen.',
        'The article uses the page only to describe the dated first-party booking transaction and free no-obligation boundary; it does not claim a guaranteed slot, eligibility, value, offer, payment, closing, or transaction result.',
      ],
    ],
    [
    ...
    [
      ...
      [
        'The current MRX How It Works page describes the evidence-organizing review process that can follow booking, including knowns, unknowns, sources, owner choices, and possible professional routing.',
        'The article uses the page only to distinguish appointment booking from later review preparation and review scope; it does not establish ownership, reserves, value, tax treatment, or a transaction result.',
      ],
    ],
    [
      'https://mineralrightsxchange.com/privacy-policy/',
      [
        'The current MRX privacy policy describes first-party information handling, account and communication data, security controls, retention choices, and user privacy options.',
        'The article uses the policy only for the first-party privacy and minimal-data boundary; it does not make an absolute security guarantee or infer an owner-specific legal right.',
      ],
    ],
    [
...
if (
  ...
  throw new Error(
    'Wave 52 review inputs do not satisfy identity, article-depth, source, or creative gates',
  );
}
...
writeArtifact('editorial', `${programRowId}-${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave52_editorial',
  review_run_id: `mrx1000-wave52-editorial-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'editorial',
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; the answer-first article contains ${wordCount} body tokens and five FAQs.`,
    'The article owns the current booking transaction. It covers private assistant entry, timezone and timing-window selection, live slot choice, appointment contact details, separate permissions, booked-versus-not-booked states, failure recovery, and routes broader review preparation, fee, and substantive review-process jobs to established pages.',
...
writeArtifact('factual_citation', `${slug}.review.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave52_factual',
  review_run_id: `mrx1000-wave52-factual-${reviewedAt.replace(/[-:]/g, '')}`,
  ...
  findings: [
    ...
    'MRX claims remain bounded to the current booking, FAQ, How It Works, privacy, and terms pages. The article dates the changeable interface, distinguishes selected from confirmed status, keeps requested-call and confirmation choices separate, and treats an availability or confirmation failure as not booked.',
...
writeArtifact('compliance', `${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave52_compliance',
  review_run_id: `mrx1000-wave52-compliance-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'compliance',
  expected_hero_sha256: heroSha,
  expected_inline_sha256: inlineSha,
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; hero/share and inline image hashes are separately locked.`,
    'The article states educational and professional boundaries, distinguishes a selected slot from a confirmed appointment, keeps requested-call, email, SMS, optional AI-voice, and marketing choices separate, uses minimal appointment data, and makes no owner-specific title, value, legal, tax, consent, or transaction conclusion.',
    'Image text is limited to the exact article title and exact canonical keyword and adds no owner data, contact detail, calendar date, appointment time, buyer endorsement, property identifier, price, consent conclusion, guarantee, value result, tax outcome, or transaction conclusion.',
  ],
  checks: [
    'complete_file_sha256_match',
...
console.log(
  `Built three hash-locked Wave 52 review artifacts with ${sources.length} live source checks.`,
);
