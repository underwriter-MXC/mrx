...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 74 admission argument(s): ${unknownArgs.join(', ')}`);
}
...
const keyword = 'mineral rights document redaction checklist';
const heroFilename = 'mineral-rights-document-redaction-checklist-before-you-share-records';
const heroAlt =
...
const decisionRelativePath = 'docs/governance/mrx1000-wave74-selection-decision-2026-08-22.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave74-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave74_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W74-SELECT-2026-08-22';
if (
  ...
  throw new Error('Wave 74 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave74' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave74-distinct-generated-overhead-redaction-checklist',
...
if (sourceRow.canonical_slug === priorCanonicalSlug) {
  Object.assign(sourceRow, {
    ...
    secondary_keywords: [keyword, 'redact mineral rights documents', 'mineral rights share copy'],
    ...
    action_reason:
      'The planning identity promised confidentiality and would duplicate generic privacy, intake, and seller-protection material while making an unsupported outcome claim. The approved replacement owns only an original-preservation, separate-share-copy, and redaction-log workflow for one defined request. It cannot promise confidentiality or security, verify a recipient, interpret law, decide required disclosure, conceal a material fact, or replace a complete record. Release remains controlled by the signed batch, matching evidence, deployment, and live verification.',
    ...
    dedupe_evidence: {
      ...
      nearest_same_cluster_title_token_jaccard: 0.1429,
      review_status: 'exact_and_fuzzy_title_check_pass',
    },
    exact_slug_unique: true,
    exact_title_unique: true,
    nearest_same_cluster_slug: 'how-to-build-a-mineral-rights-sale-document-package-index',
    cannibalization_score: 0.1429,
...
if (
  ...
  throw new Error('Wave 74 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
const articleSha = sha256(articleBytes);
ledger.identity_registry ??= {};
const wave74Rekey = ledger.identity_registry.wave74_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: priorCanonicalSlug,
  prior_source_handle: 'searchatlas-topical-map:254654:title:290f3a76-9751-4a35-b5c0-a095733786fa',
...
if (wave74Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 74 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave74_rekey = wave74Rekey;
wave74Rekey.draft_selection_decision_sha256 ??= wave74Rekey.selection_decision_sha256;
wave74Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave74Rekey.release_candidate_article_sha256 = articleSha;
wave74Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The original identity was rejected because it promised confidentiality and overlapped generic privacy, intake, and seller-protection material. The admitted replacement owns only preservation of the original, creation of one purpose-limited share copy, and a redaction log that never repeats the hidden value.',
    'MRX, NIST, FTC, and IRS sources support only bounded document-handling, PII-context, unexpected-request, identity-theft-context, and W-9 purpose facts. None promises confidentiality or security, verifies a recipient, interprets law, decides required disclosure, or proves that redaction prevents misuse.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W74-SELECT-2026-08-22; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 74 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave74_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave74_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave74_selection_decision_sha256 = sha256(decisionBytes);
