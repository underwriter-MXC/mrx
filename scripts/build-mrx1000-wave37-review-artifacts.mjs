...
const programRowId = 'MRX1000-0117';
const title = 'Top Five Missteps to Avoid for a Successful Mineral Rights Sale';
const inlineKeyword = 'What Are Common Pitfalls I Should Avoid in Selling Mineral Rights?';
const articlePath = `src/content/posts/${slug}.mdx`;
const creativePath = `artifacts/mrx1000-wave37-creative-qa/${slug}/creative-manifest.json`;
...
if (!row || row.slug !== slug || row.title !== title) {
  throw new Error('Wave 37 batch identity is missing or drifted');
}
...
if (
  ...
  throw new Error(
    'Wave 37 review inputs do not satisfy identity, article-depth, source, or creative gates',
  );
}
...
writeArtifact('editorial', `${programRowId}-${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave37_editorial',
  review_run_id: `mrx1000-wave37-editorial-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'editorial',
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; the answer-first article contains ${wordCount} body tokens and five FAQs.`,
    'The article owns a five-error owner decision and recovery job: it separates objective and scope, evidence readiness, comparable proposal review, professional handoffs, and controlled closing, then provides a prevention artifact, recovery action, stop rule, and bounded professional question for each misstep.',
...
writeArtifact('factual_citation', `${slug}.review.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave37_factual',
  review_run_id: `mrx1000-wave37-factual-${reviewedAt.replace(/[-:]/g, '')}`,
...
writeArtifact('compliance', `${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave37_compliance',
  review_run_id: `mrx1000-wave37-compliance-${reviewedAt.replace(/[-:]/g, '')}`,
...
console.log(
  `Built three hash-locked Wave 37 review artifacts with ${sources.length} live source checks.`,
);
