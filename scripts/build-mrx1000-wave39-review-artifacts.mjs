...
const title =
  'Uncovering the Truth: Are There Any Fees for Your No-Obligation Mineral Rights Assessment?';
const inlineKeyword = 'Are There Any Fees Associated With Getting a No-Obligation Assessment?';
const articlePath = `src/content/posts/${slug}.mdx`;
const creativePath = `artifacts/mrx1000-wave39-creative-qa/${slug}/creative-manifest.json`;
...
if (!row || row.slug !== slug || row.title !== title) {
  throw new Error('Wave 39 batch identity is missing or drifted');
}
...
function sourceScope(source) {
  const scopes = new Map([
    ...
    [
      ...
      [
        'The current MRX How It Works page describes an owner-controlled sequence and an educational, directional review that is free, requires no card, and creates no obligation to sell.',
        'The article uses the page only for the current sequence and directional-deliverable boundary; it does not establish value, title, transaction terms, or an owner-specific result.',
      ],
    ],
    [
    ...
    [
      ...
      [
        'The current MRX Privacy Policy is the first-party description of MRX information practices and the choices or contact routes it publishes.',
        'The article uses the policy only to direct readers to the current first-party information-practice statement; it does not interpret the policy, determine privacy rights, or expand the disclosed practices.',
      ],
    ],
    [
      'https://mineralrightsxchange.com/terms/',
      [
        'The current MRX terms state that website information is educational rather than owner-specific professional advice and that booking a requested phone call does not require consent to marketing texts.',
        'The article uses the terms only to preserve the educational boundary and separate requested-call communications from optional marketing choices; it does not interpret a consent control, agreement, or owner’s rights.',
      ],
    ],
  ]);
...
if (
  ...
  throw new Error(
    'Wave 39 review inputs do not satisfy identity, article-depth, source, or creative gates',
  );
}
...
writeArtifact('editorial', `${programRowId}-${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave39_editorial',
  review_run_id: `mrx1000-wave39-editorial-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'editorial',
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; the answer-first article contains ${wordCount} body tokens and five FAQs.`,
    'The article owns the assessment-request service-boundary verification job: it verifies the current zero-fee, no-card, and no-obligation policy; separates appointment, transactional, marketing, document-follow-up, outside-service, and later-transaction decisions; and supplies a ten-field dated boundary receipt with explicit pause rules.',
...
writeArtifact('factual_citation', `${slug}.review.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave39_factual',
  review_run_id: `mrx1000-wave39-factual-${reviewedAt.replace(/[-:]/g, '')}`,
  ...
  findings: [
    ...
    'MRX claims remain bounded to its current FAQ, booking, How It Works, methodology, Privacy Policy, and Terms pages; the article does not extend the first-party assessment promise to outside services or later transaction documents.',
...
writeArtifact('compliance', `${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave39_compliance',
  review_run_id: `mrx1000-wave39-compliance-${reviewedAt.replace(/[-:]/g, '')}`,
...
console.log(
  `Built three hash-locked Wave 39 review artifacts with ${sources.length} live source checks.`,
);
