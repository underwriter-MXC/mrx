...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 68 admission argument(s): ${unknownArgs.join(', ')}`);
}
...
const decisionRelativePath = 'docs/governance/mrx1000-wave68-selection-decision-2026-08-22.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave68-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave68_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W68-SELECT-2026-08-22';
if (
  ...
  throw new Error('Wave 68 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave68' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave68-distinct-generated-overhead-property-scope-crosswalk-workspace',
...
const priorCanonicalTitle = 'What You Must Know Before Evaluating Mineral Rights';
const priorCanonicalSlug = 'what-you-must-know-before-evaluating-mineral-rights';
if (
  !sourceRow ||
...
if (sourceRow.canonical_slug === priorCanonicalSlug) {
  ...
  action_reason:
    'The replacement property-scope-crosswalk article was selected after the original evaluation identity collided with the live valuation and sale-decision corpus, a facts-versus-assumptions proposal collided with Article 133, and a first-response proposal collided with Article 147. It owns only the pre-comparison administrative transcription of stated property and interest scope and remains controlled by the signed batch, matching evidence, deployment, and live verification.',
  ...
  dedupe_evidence: {
    ...
    nearest_same_cluster_slug: 'calculate-price-per-net-mineral-acre',
    nearest_same_cluster_title_token_jaccard: 0.1875,
    review_status: 'exact_and_fuzzy_title_check_pass',
  },
  exact_slug_unique: true,
  exact_title_unique: true,
  nearest_same_cluster_slug: 'calculate-price-per-net-mineral-acre',
  cannibalization_score: 0.1875,
...
if (
  ...
  throw new Error('Wave 68 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
const articleSha = sha256(articleBytes);
ledger.identity_registry ??= {};
const wave68Rekey = ledger.identity_registry.wave68_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: priorCanonicalSlug,
  prior_searchatlas_handle:
    'searchatlas-topical-map:255581:title:6efdac42-7968-46a0-b328-2f666cd86b06',
...
if (wave68Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 68 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave68_rekey = wave68Rekey;
wave68Rekey.draft_selection_decision_sha256 ??= wave68Rekey.selection_decision_sha256;
wave68Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave68Rekey.release_candidate_article_sha256 = articleSha;
wave68Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The original evaluation identity was rejected because live articles already own valuation, assessment scope, sale decisions, price-per-net-mineral-acre, and offer review. Facts-versus-assumptions and first-response replacements were also rejected for collisions with Articles 133 and 147. The admitted replacement owns only a pre-comparison administrative crosswalk of stated property and interest scope.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W68-SELECT-2026-08-22; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 68 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave68_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave68_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave68_selection_decision_sha256 = sha256(decisionBytes);
