...
const reviewedAt = '2026-08-06T13:00:00Z';
const decisionId = 'MRX1000-W9-SELECT-2026-08-06';

if (rows.length !== 10) throw new Error(`Expected exactly ten Wave 9 rows; found ${rows.length}`);
...
async function verifySource(url) {
  const response = await fetch(url, {
    redirect: 'follow',
    headers: { 'user-agent': 'MRX-Codex-Source-Verifier/1.0', 'cache-control': 'no-cache' },
  });
  return {
    requested_url: url,
...
for (const row of rows) {
  ...
  for (const declared of sources) {
    ...
    if (!result.pass) throw new Error(`${row.slug}: source ${declared.url} returned ${result.status}`);
  ...
  writeArtifact('editorial', `${row.program_row_id}-${row.slug}.json`, {
    ...
    ...common,
    reviewer_id: 'codex_editorial_wave9',
    review_run_id: 'mrx1000-wave9-continuous-editorial-20260806',
  ...
  writeArtifact('factual_citation', `${row.slug}.review.json`, {
    ...
    ...common,
    reviewer_id: 'codex_factual_wave9',
    review_run_id: 'mrx1000-wave9-continuous-factual-20260806',
    ...
    findings: [
      ...
      'MRX pages are used only as first-party evidence of the current methodology and intake statements, not as independent proof of value, market coverage, or eligibility.',
    ],
    checks: [
    ...
    sources_inspected: sourceAccess.map((entry) => ({
      ...
      http_access_result: { status: entry.status, final_url: entry.final_url, content_type: entry.content_type },
      ...
      claim_scope: 'Owner-specific legal, tax, title, accounting, brokerage, fraud, engineering, appraisal, eligibility, value, and transaction conclusions remain outside this source use.',
    })),
  });
  ...
  writeArtifact('compliance', `${row.slug}.json`, {
    ...
    ...common,
    reviewer_id: 'codex_compliance_wave9',
    review_run_id: 'mrx1000-wave9-continuous-compliance-20260806',
...
console.log('Built 30 hash-locked Wave 9 review artifacts with live source checks.');
