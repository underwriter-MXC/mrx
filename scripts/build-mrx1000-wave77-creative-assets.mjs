...
const inlineFilename = 'mineral-rights-worksheet-question-locator';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave77-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave77-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave77-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Premium photorealistic people-free side-angle review desk with an open blank worksheet, one blank callout bracket, and a separate blank question card on the right; quiet dark field on the left; no maps, values, charts, readable words, names, identifiers, signatures, forms, logos, promises, or fake document text.',
  inline:
    'Materially distinct people-free strict overhead pale locator-board scene with one outlined blank worksheet cell, a separate blank source-reference tag, and a separate blank question-routing card above a quiet lower navy field; no values, charts, readable words, identifiers, signatures, forms, logos, promises, or fake document text.',
};
  ...
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="title" x="46" y="168">
        <tspan x="46" dy="0">How to Locate an</tspan>
        <tspan x="46" dy="51">Unanswered Question</tspan>
        <tspan x="46" dy="51">in a Mineral Rights</tspan>
        <tspan x="46" dy="51">Worksheet</tspan>
      </text>
    </svg>
  `);
}
  ...
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="keyword" x="600" y="536">
        <tspan x="600" dy="0">mineral rights worksheet</tspan>
        <tspan x="600" dy="54">question locator</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave77-ocr-'));
  try {
