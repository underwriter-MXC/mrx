...
const inlineKeyword = 'Mineral Rights Sale Decision Log';
const heroAlt =
...
const creativePath = `artifacts/mrx1000-wave64-creative-qa/${slug}/creative-manifest.json`;
...
const sourceReceiptPath = process.env.MRX_WAVE64_SOURCE_RECEIPTS_PATH ?? null;
...
if (!row || row.slug !== slug || row.title !== title || row.selection_rank !== 144) {
  throw new Error('Wave 64 batch identity is missing or drifted');
}
...
async function verifySource({ label, url }) {
  if (sourceReceiptBundle) {
    ...
    if (!Number.isFinite(checkedAt) || ageMs < -300_000 || ageMs > 7_200_000) {
      throw new Error('Wave 64 preverified source receipts are invalid, future-dated, or stale');
    }
...
const sourceScopes = new Map([
  [
    ...
    [
      'The current MRX methodology describes a directional range with stated evidence, assumptions, scenarios, and professional limits rather than a teaser number or certified valuation.',
      'The article uses it only to explain why a decision log should keep source records, reported facts, assumptions, conflicts, scenarios, and professional conclusions distinct; it does not establish title, value, legal or tax treatment, an offer, owner net, or a sell decision.',
    ],
  ],
  [
    'https://mineralrightsxchange.com/faq/',
    [
      'The current MRX FAQ describes the service as a directional underwriter assessment and distinguishes it from a certified valuation, legal opinion, or tax opinion.',
      'The article uses it only to label professional-referral states and first-party scope limits; it does not establish owner-specific title, value, taxes, suitability, an offer, payment, closing, or a sell-versus-hold recommendation.',
    ],
  ],
  [
    'https://mineralrightsxchange.com/terms/',
    [
      'The current MRX terms state that public content is educational and is not a certified appraisal, title opinion, reserve certification, or individualized professional conclusion.',
      'The article uses it only to keep an organizational log from being presented as a professional or transaction conclusion; it does not derive owner-specific legal, tax, title, appraisal, engineering, value, offer, or transaction conclusions.',
    ],
  ],
  [
    ...
    [
      ...
      'The article uses it only to distinguish a potentially recordable instrument from a private process log; it does not determine the validity, recordability, legal effect, ownership result, or sufficiency of any document.',
    ],
  ],
  [
    'https://www.sos.state.tx.us/corp/options.shtml',
    [
      'The Texas Secretary of State describes online services that can search business entities, obtain entity information, and order copies or certificates.',
      'The article uses it only as an example of recording the exact entity name, official source, access date, and result type; it does not endorse a counterparty or establish authority, financial capacity, reliability, or transaction quality.',
    ],
  ],
  [
    'https://www.rrc.texas.gov/resource-center/research/research-queries/',
    [
      'The Railroad Commission of Texas provides research queries for production, wells, permits, operators, and related records and states that its continually updated data sets are informational, non-authoritative, and have no legal force or effect.',
      'The article uses it only as an example of preserving query fields, reporting period, access date, export identity, and source limitations; it does not treat the data as title, ownership, acreage, reserves, value, or transaction advice.',
    ],
  ],
]);
...
if (
  ...
  throw new Error(
    'Wave 64 review inputs do not satisfy identity, article-depth, source, or creative gates',
  );
}
...
writeArtifact('editorial', `${programRowId}-${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave64_editorial',
  review_run_id: `mrx1000-wave64-editorial-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'editorial',
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; the answer-first article contains ${wordCount} body tokens and five FAQs.`,
    'The article owns the narrow decision-log construction job for owners already in a possible sale process. It defines a stable header, dated entry fields, evidence classes, version and supersession rules, escalation states, and privacy limits while stopping before misstep lists, sale-process narration, offer comparison, contract interpretation, value, and transaction selection.',
...
writeArtifact('factual_citation', `${slug}.review.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave64_factual',
  review_run_id: `mrx1000-wave64-factual-${reviewedAt.replace(/[-:]/g, '')}`,
  ...
  findings: [
    ...
    'Claims remain bounded to MRX evidence and professional-limit language, Texas recording-law context, Texas Secretary of State entity-record services, and RRC informational-query limitations. No source is converted into private title, document validity, counterparty endorsement, reserves, offer terms, owner net, legal or tax treatment, or transaction advice.',
...
writeArtifact('compliance', `${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave64_compliance',
  review_run_id: `mrx1000-wave64-compliance-${reviewedAt.replace(/[-:]/g, '')}`,
  ...
  findings: [
    ...
    'Image text is limited to the exact article title and approved decision-log phrase and adds no account data, property identifier, amount, date, equation, official seal, title conclusion, value conclusion, recommendation, offer, guarantee, buyer endorsement, or transaction result.',
  ],
  checks: [
    'complete_file_sha256_match',
...
console.log(
  `Built three hash-locked Wave 64 review artifacts with ${sources.length} current source checks.`,
);
