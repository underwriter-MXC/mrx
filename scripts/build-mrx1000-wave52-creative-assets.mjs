...
const keyword = 'What Steps Are Involved in Getting a No-Obligation Review of My Mineral Rights?';
const inlineFilename =
  'what-steps-are-involved-in-getting-a-no-obligation-review-of-my-mineral-rights';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave52-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave52-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave52-creative-qa/${slug}`),
};

const generationPrompts = {
  hero:
    'No-people private mineral-rights phone-appointment desk with blank phone interface, three blank slot cards, timezone clocks, privacy shield folder, mineral core, Texas contour, and a pumpjack beyond the window beside a deep-navy exact-title field; no extra readable words, numbers, prices, dates, times, signatures, private information, logos, legal conclusions, ownership conclusions, value conclusions, buyer claims, or success claims.',
  inline:
    'Compositionally distinct seven-stage tactile booking workflow showing assistant entry, timezone, timing window, three slots, contact details, separate permissions, and on-screen confirmation below a navy exact-keyword field; no extra readable words, numbers, prices, dates, times, signatures, private information, logos, legal conclusions, ownership conclusions, value conclusions, buyer claims, or success claims.',
};
...
function heroTypography() {
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <style>
        .title { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 37px; font-weight: 700; letter-spacing: -0.55px; }
        .rule { fill: #d79a2b; }
      </style>
      <rect x="0" y="0" width="610" height="630" fill="url(#navy)" />
      <rect class="rule" x="46" y="104" width="82" height="5" rx="2.5" />
      <text class="title" x="46" y="178">
        <tspan x="46" dy="0">Understanding the</tspan>
        <tspan x="46" dy="50">Steps to Book Your</tspan>
        <tspan x="46" dy="50">Free No-Obligation</tspan>
        <tspan x="46" dy="50">Mineral Rights Review</tspan>
      </text>
    </svg>
  `);
}
...
function inlineTypography() {
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <style>
        .keyword { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 38px; font-weight: 700; text-anchor: middle; letter-spacing: -0.3px; }
        .rule { fill: #d79a2b; }
      </style>
      <rect x="0" y="0" width="1200" height="250" fill="url(#navy)" />
      <rect class="rule" x="0" y="246" width="1200" height="4" />
      <text class="keyword" x="600" y="74">
        <tspan x="600" dy="0">What Steps Are Involved</tspan>
        <tspan x="600" dy="49">in Getting a No-Obligation</tspan>
        <tspan x="600" dy="49">Review of My Mineral Rights?</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave52-ocr-'));
  try {
