...
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave31-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave31-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave31-creative-qa/${slug}`),
};

const generationPrompts = {
  hero:
    'Photorealistic mineral owner and professional guide reviewing a blank dependency-card path in a warm ranch office, with a dark title-safe panel and no readable words, letters, numbers, dates, or clocks.',
  inline:
    'Compositionally distinct no-people overhead dependency path with seven blank pictogram cards, brass connectors, a central keyword-safe folder, one reset card, and no readable words, letters, numbers, dates, or clocks.',
};
...
function heroTypography() {
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      <style>
        .title { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 60px; font-weight: 700; letter-spacing: -1px; }
        .rule { fill: #d79a2b; }
      </style>
      <rect class="rule" x="72" y="113" width="72" height="5" rx="2.5" />
      <text class="title" x="72" y="190">
        <tspan x="72" dy="0">Selling Mineral</tspan>
        <tspan x="72" dy="66">Rights: Timeline</tspan>
        <tspan x="72" dy="66">In 2026</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave31-ocr-'));
  try {
