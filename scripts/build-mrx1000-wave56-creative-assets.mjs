...
const keyword = 'What Is the Typical Pricing Range for Mineral Rights in My Area?';
const inlineFilename =
  'what-is-the-typical-pricing-range-for-mineral-rights-in-my-area';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave56-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave56-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave56-creative-qa/${slug}`),
};

const generationPrompts = {
  hero:
    'Calm mineral-owner local-range workspace with an owner and underwriter, a mapped tract, blurred royalty pages, an unlabeled production curve, three evidence bands, and a deep-navy exact-title field; no extra readable words, numbers, prices, percentages, dates, signatures, private information, logos, legal conclusions, ownership conclusions, value conclusions, buyer claims, or success claims.',
  inline:
    'Compositionally distinct overhead local-range evidence table with a tract map, blurred royalty pages, an unlabeled production chart, a plain evidence folder, three comparison bands, two sorting hands, and a centered navy exact-question card; no extra readable words, numbers, prices, percentages, dates, signatures, private information, logos, legal conclusions, ownership conclusions, value conclusions, buyer claims, or success claims.',
};
...
function heroTypography() {
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="title" x="46" y="185">
        <tspan x="46" dy="0">Understanding Typical</tspan>
        <tspan x="46" dy="58">Pricing Ranges for</tspan>
        <tspan x="46" dy="58">Mineral Rights in Your</tspan>
        <tspan x="46" dy="58">Area: What to Expect</tspan>
      </text>
    </svg>
  `);
}
...
function inlineTypography() {
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect x="240" y="165" width="720" height="345" rx="18" fill="url(#navy)" />
      <rect class="rule" x="365" y="217" width="470" height="4" rx="2" />
      <text class="keyword" x="600" y="280">
        <tspan x="600" dy="0">What Is the Typical</tspan>
        <tspan x="600" dy="53">Pricing Range for</tspan>
        <tspan x="600" dy="53">Mineral Rights in</tspan>
        <tspan x="600" dy="53">My Area?</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave56-ocr-'));
  try {
