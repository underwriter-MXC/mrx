...
const inlineFilename = 'mineral-rights-sale-package-index';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave65-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave65-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave65-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Premium documentary mineral-rights transaction document package on a dark walnut desk with an open archival binder, blank table-of-contents sheet, tab dividers, unsigned pages, subtle parcel-map context, brass paper clips, and a magnifying glass on the right beside a deep-navy exact-title field; no people, cash, buyer branding, fake forms, extra readable words, account numbers, dollar figures, dates, signatures, private information, logos, title conclusions, value conclusions, legal conclusions, recommendations, offers, guarantees, or success claims.',
  inline:
    'Compositionally distinct people-free overhead package-index workspace on pale stone with blank overlapping document folders, visible tabs, a closed navy folder, empty signature-copy envelope, page counter, teal index card, and a lower navy exact-phrase field; no map, binder rings, magnifier, cash, buyer branding, fake forms, extra readable words, account numbers, dollar figures, dates, signatures, private information, logos, title conclusions, value conclusions, legal conclusions, recommendations, offers, guarantees, or success claims.',
};
...
function heroTypography() {
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <style>
        .title { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 43px; font-weight: 700; letter-spacing: -0.35px; }
        .rule { fill: #d79a2b; }
      </style>
      ...
      <text class="title" x="46" y="205">
        ...
        <tspan x="46" dy="66">Mineral Rights Sale</tspan>
        <tspan x="46" dy="66">Document Package Index</tspan>
      </text>
    </svg>
  `);
}
  ...
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="keyword" x="600" y="558">
        <tspan x="600" dy="0">Mineral Rights Sale Package Index</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave65-ocr-'));
  try {
