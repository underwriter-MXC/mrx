...
const titleLines = ['Discount Rates in', 'Mineral Rights DCF:', 'A Plain-Language Guide'];
const keyword = 'discount rates in mineral rights DCF';

const paths = {
  heroSource: join(
    root,
    'artifacts/mrx1000-wave19-creative-sources/discount-rates-in-mineral-rights-dcf-a-plain-language-guide-hero-base.png',
  ),
  inlineSource: join(
    root,
    'artifacts/mrx1000-wave19-creative-sources/discount-rates-in-mineral-rights-dcf-a-plain-language-guide-inline-base.png',
  ),
  hero: join(
    root,
    'public/assets/articles/hero/discount-rates-in-mineral-rights-dcf-a-plain-language-guide.webp',
  ),
  inline: join(
    root,
    'public/assets/articles/inline/discount-rates-in-mineral-rights-dcf-a-plain-language-guide/discount-rates-in-mineral-rights-dcf.webp',
  ),
  qa: join(
    root,
    'artifacts/mrx1000-wave19-creative-qa/discount-rates-in-mineral-rights-dcf-a-plain-language-guide',
  ),
};

const generationPrompts = {
  hero:
    'Photorealistic West Texas mineral-valuation workspace with blurred royalty statements, an unlabelled physical cash-flow timeline, calculator, rock core, and distant oilfield infrastructure at dusk; clean dark left-side title space; no readable words, numbers, labels, logos, identifiers, claims, or owner-specific data.',
  inline:
    'Distinct photorealistic overhead present-value workspace with unlabelled annual tiles, diminishing neutral token stacks, magnifying glass, rock sample, and analog slider; clean right-side keyword space; no words, numbers, equations, labels, identifiers, claims, or owner-specific data.',
};
...
function inlineOverlay() {
  return Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="675">
    ...
    <text x="714" y="434" fill="#fffdf8" font-family="Arial,Helvetica,sans-serif" font-size="38" font-weight="700" letter-spacing="-.6" filter="url(#inline-shadow)">
      <tspan x="714" y="434">discount rates in</tspan>
      <tspan x="714" y="490">mineral rights DCF</tspan>
    </text>
  </svg>`);
}
...
async function main() {
  ...
  if (basename(paths.hero, '.webp') !== 'discount-rates-in-mineral-rights-dcf-a-plain-language-guide') {
    throw new Error('Hero filename does not match the exact-title slug');
  }
  if (basename(paths.inline, '.webp') !== 'discount-rates-in-mineral-rights-dcf') {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave19-ocr-'));
  try {
    ...
    const manifest = {
      ...
      article: {
        ...
        hero: {
          ...
          public_path: '/assets/articles/hero/discount-rates-in-mineral-rights-dcf-a-plain-language-guide.webp',
        ...
        inline: {
          ...
          public_path:
            '/assets/articles/inline/discount-rates-in-mineral-rights-dcf-a-plain-language-guide/discount-rates-in-mineral-rights-dcf.webp',
