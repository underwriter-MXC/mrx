...
const slug = 'top-five-missteps-to-avoid-for-a-successful-mineral-rights-sale';
const title = 'Top Five Missteps to Avoid for a Successful Mineral Rights Sale';
const keyword = 'What Are Common Pitfalls I Should Avoid in Selling Mineral Rights?';
const inlineFilename = 'what-are-common-pitfalls-i-should-avoid-in-selling-mineral-rights';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave37-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave37-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave37-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Photorealistic mineral owner organizing five decision folders, a property map, calculator, and pen, with a dark navy title-safe panel on the left and no readable words, letters, numbers, prices, percentages, formulas, signatures, logos, or seals.',
  inline:
    'Compositionally distinct no-people overhead five-checkpoint prevention system with five connected cards, a property map, magnifier, balance, document clip, shield, pen, and envelope, plus a lower keyword-safe field and no readable words, letters, numbers, prices, percentages, formulas, signatures, logos, or seals.',
};
...
function heroTypography() {
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      <style>
        .title { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 43px; font-weight: 700; letter-spacing: -1px; }
        .rule { fill: #d79a2b; }
      </style>
      <rect class="rule" x="72" y="145" width="72" height="5" rx="2.5" />
      <text class="title" x="72" y="210">
        <tspan x="72" dy="0">Top Five Missteps</tspan>
        <tspan x="72" dy="52">to Avoid for a</tspan>
        <tspan x="72" dy="52">Successful Mineral</tspan>
        <tspan x="72" dy="52">Rights Sale</tspan>
      </text>
    </svg>
  `);
}
...
function inlineTypography() {
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect class="rule" x="533" y="448" width="134" height="5" rx="2.5" />
      <text class="keyword" x="600" y="506">
        <tspan x="600" dy="0">What Are Common Pitfalls</tspan>
        <tspan x="600" dy="49">I Should Avoid in Selling</tspan>
        <tspan x="600" dy="49">Mineral Rights?</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave37-ocr-'));
  try {
