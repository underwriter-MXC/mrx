...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 81 admission argument(s): ${unknownArgs.join(', ')}`);
}
...
const keyword = 'post-production cost evidence packet royalty records';
const heroFilename = 'post-production-cost-evidence-packet-for-royalty-records';
const heroAlt =
...
const decisionRelativePath = 'docs/governance/mrx1000-wave81-selection-decision-2026-08-23.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave81-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave81_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W81-SELECT-2026-08-23';
if (
  ...
  throw new Error('Wave 81 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave81' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave81-distinct-generated-overhead-royalty-record-source-matrix',
...
const priorCanonicalTitle = 'Post-Production Costs and Their Effect on Royalty Cash Flow';
const priorCanonicalSlug = 'post-production-costs-and-their-effect-on-royalty-cash-flow';
if (
  !sourceRow ||
...
if ([priorCanonicalSlug, slug].includes(sourceRow.canonical_slug)) {
  Object.assign(sourceRow, {
    ...
    secondary_keywords: [keyword, 'royalty deduction evidence packet', 'royalty statement source log'],
    ...
    action_reason:
      'The original post-production-cost and royalty-cash-flow explainer identity overlapped the live statement-decoding, royalty-reconciliation, lease-rate, basis-differential, monthly-history, and valuation-methodology corpus. The approved replacement owns only source-preserving capture of reported cost entries, source location, payor and property identifiers as displayed, product and period, related support, evidence status, and neutral questions. It uses only matched, conflict, missing, or unresolved and stops before statement decoding, lease interpretation, deduction-permissibility judgments, royalty auditing, payment resolution, owner-decimal validation, cash-flow calculation, valuation, or transaction conclusions. Release remains controlled by the signed batch, matching evidence, deployment, and live verification.',
    ...
    dedupe_evidence: {
      ...
      nearest_same_cluster_slug: 'understanding-your-mineral-royalty-checks-breakdown',
      nearest_same_cluster_title_token_jaccard: 0.1538,
      review_status: 'exact_and_fuzzy_title_check_pass',
    },
    exact_slug_unique: true,
    exact_title_unique: true,
    nearest_same_cluster_slug: 'understanding-your-mineral-royalty-checks-breakdown',
    cannibalization_score: 0.1538,
...
if (
  ...
  throw new Error('Wave 81 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
const articleSha = sha256(articleBytes);
ledger.identity_registry ??= {};
const wave81Rekey = ledger.identity_registry.wave81_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: priorCanonicalSlug,
  prior_source_handle:
    'factory-taxonomy-synthesis:valuation:post-production-costs-royalty-cash-flow',
...
if (wave81Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 81 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave81_rekey = wave81Rekey;
wave81Rekey.draft_selection_decision_sha256 ??= wave81Rekey.selection_decision_sha256;
wave81Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave81Rekey.release_candidate_article_sha256 = articleSha;
wave81Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The original post-production-cost and royalty-cash-flow explainer identity was rejected because it overlaps the live statement-decoding, royalty-reconciliation, lease-rate, basis-differential, monthly-history, and valuation-methodology corpus. The admitted replacement owns only a source-preserving evidence packet for reported entries and related support.',
    'Current official Texas Legislature, Railroad Commission, and Comptroller sources support only bounded descriptions of payment-information context, production-query limits, and separate tax-administration source lanes. None interprets a private instrument, classifies a reported entry, validates an owner payment, establishes liability, calculates cash flow, or determines an offer or value.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W81-SELECT-2026-08-23; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 81 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave81_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave81_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave81_selection_decision_sha256 = sha256(decisionBytes);
