...
const rows = batch.articles.filter((row) => row.selection_rank >= 41 && row.selection_rank <= 50);
const reviewedAt = '2026-08-06T22:45:00Z';
const decisionId = 'MRX1000-W6-SELECT-2026-08-06';

if (rows.length !== 10) throw new Error(`Expected exactly ten Wave 6 rows; found ${rows.length}`);
...
for (const row of rows) {
  ...
  writeArtifact('editorial', `${row.program_row_id}-${row.slug}.json`, {
    ...
    ...common,
    reviewer_id: 'codex_editorial_wave6',
    review_run_id: 'mrx1000-wave6-continuous-editorial-20260806',
    capability: 'editorial',
    findings: [
      `Complete reviewed MDX SHA-256 is ${inputBodySha}; the article is answer-first, contains ${wordCount} rendered-word tokens, and has exactly five distinct FAQs.`,
      'Headings, tables or checklists, caveats, owner actions, source notes, and internal-link progression serve the selected search job without merging the neighboring article intent.',
      'The exact canonical title is used in the page, hero alt, and social alt, with one unique 1200-by-630 WebP bound to both hero and share metadata.',
    ],
    checks: [
  ...
  writeArtifact('factual_citation', `${row.slug}.review.json`, {
    ...
    ...common,
    reviewer_id: 'codex_factual_wave6',
    review_run_id: 'mrx1000-wave6-continuous-factual-20260806',
    capability: 'factual_citation',
    findings: [
      `Complete reviewed MDX SHA-256 is ${inputBodySha}; all ${sources.length} declared HTTPS sources returned HTTP 200 in the 2026-08-06 source-access check.`,
      'Texas legal and regulatory claims are bounded to official Texas statutes or agency pages; federal tax education is bounded to current IRS primary guidance.',
      'MRX methodology pages are treated only as first-party evidence of the stated process and limitations, not independent market-value proof.',
    ],
    checks: [
    ...
    sources_inspected: sources.map(({ label, url }) => ({
      ...
      accessed_at: '2026-08-06T22:40:00Z',
      ...
      claim_scope: `Owner-specific legal, probate, tax, title, accounting, lease-status, and certified-valuation conclusions remain outside this source use.`,
    })),
  });
  ...
  writeArtifact('compliance', `${row.slug}.json`, {
    ...
    ...common,
    reviewer_id: 'codex_compliance_wave6',
    review_run_id: 'mrx1000-wave6-continuous-compliance-20260806',
    ...
    checks: [
      ...
      'no_guaranteed_result_money_or_timing_claim',
...
console.log('Built 30 hash-locked Wave 6 review artifacts.');
