...
const rows = batch.articles.filter((row) => row.selection_rank >= 61 && row.selection_rank <= 70);
const reviewedAt = '2026-08-06T11:45:00Z';
const decisionId = 'MRX1000-W8-SELECT-2026-08-06';

if (rows.length !== 10) throw new Error(`Expected exactly ten Wave 8 rows; found ${rows.length}`);
...
for (const row of rows) {
  ...
  writeArtifact('editorial', `${row.program_row_id}-${row.slug}.json`, {
    ...
    ...common,
    reviewer_id: 'codex_editorial_wave8',
    review_run_id: 'mrx1000-wave8-continuous-editorial-20260806',
  ...
  writeArtifact('factual_citation', `${row.slug}.review.json`, {
    ...
    ...common,
    reviewer_id: 'codex_factual_wave8',
    review_run_id: 'mrx1000-wave8-continuous-factual-20260806',
    capability: 'factual_citation',
    findings: [
      `Complete reviewed MDX SHA-256 is ${inputBodySha}; all ${sources.length} declared HTTPS sources passed the 2026-08-06 source-access review.`,
      'Texas legal and regulatory claims are bounded to official Texas statutes or Railroad Commission resources; federal tax education is bounded to current IRS publications and form instructions.',
      'MRX pages are used only as first-party evidence of the current booking, fee, process, conflict, and limitation statements, not as independent proof of value or eligibility.',
    ],
    checks: [
    ...
    sources_inspected: sources.map(({ label, url }) => ({
      ...
      accessed_at: '2026-08-06T11:40:00Z',
      http_access_result: 'reachable HTTPS source or verified first-party production source',
      ...
      claim_scope: 'Owner-specific legal, probate, tax, title, accounting, lease-status, transaction, eligibility, and certified-valuation conclusions remain outside this source use.',
    })),
  });
  ...
  writeArtifact('compliance', `${row.slug}.json`, {
    ...
    ...common,
    reviewer_id: 'codex_compliance_wave8',
    review_run_id: 'mrx1000-wave8-continuous-compliance-20260806',
    ...
    findings: [
      ...
      'The article states the educational boundary and does not provide an owner-specific title, probate, trust, lease, legal, tax, accounting, audit, certified-appraisal, eligibility, or guaranteed-value conclusion.',
...
console.log('Built 30 hash-locked Wave 8 review artifacts.');
