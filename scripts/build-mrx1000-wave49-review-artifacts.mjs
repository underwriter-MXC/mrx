...
const programRowId = 'MRX1000-0129';
const title =
  'Understanding the Factors Behind the Valuation of Your Texas Mineral Rights Explained';
const inlineKeyword = 'How Is the Value of My Texas Mineral Rights Determined and Assessed?';
const articlePath = `src/content/posts/${slug}.mdx`;
const creativePath = `artifacts/mrx1000-wave49-creative-qa/${slug}/creative-manifest.json`;
...
if (!row || row.slug !== slug || row.title !== title) {
  throw new Error('Wave 49 batch identity is missing or drifted');
}
...
if (
  ...
  throw new Error(
    'Wave 49 review inputs do not satisfy identity, article-depth, source, or creative gates',
  );
}
...
writeArtifact('editorial', `${programRowId}-${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave49_editorial',
  review_run_id: `mrx1000-wave49-editorial-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'editorial',
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; the answer-first article contains ${wordCount} body tokens and five FAQs.`,
    'The article owns the Texas valuation mechanics and evidence-confidence job. It shows where interest quantity, matched owner cash flow, current production, conditional development, market and forecast assumptions, method reconciliation, and transaction scope enter a range; grades evidence separately; and routes the end-to-end process, factor-list, factor-to-range, and introductory value-driver jobs to their established pages.',
...
writeArtifact('factual_citation', `${slug}.review.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave49_factual',
  review_run_id: `mrx1000-wave49-factual-${reviewedAt.replace(/[-:]/g, '')}`,
...
writeArtifact('compliance', `${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave49_compliance',
  review_run_id: `mrx1000-wave49-compliance-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'compliance',
  expected_hero_sha256: heroSha,
  expected_inline_sha256: inlineSha,
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; hero/share and inline image hashes are separately locked.`,
    'The article states educational and professional boundaries, discloses MRX’s potential economic interest, separates the conditional range from its evidence-confidence grade and from an appraisal conclusion, written proposal, owner net, and tax record, labels input status, prohibits automatic confidence haircuts, and routes unresolved title, legal, tax, accounting, appraisal, land, engineering, geology, reserves, commodity, and transaction questions without making owner-specific conclusions.',
...
console.log(
  `Built three hash-locked Wave 49 review artifacts with ${sources.length} live source checks.`,
);
