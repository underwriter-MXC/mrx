...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 79 admission argument(s): ${unknownArgs.join(', ')}`);
}
...
const decisionRelativePath = 'docs/governance/mrx1000-wave79-selection-decision-2026-08-23.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave79-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave79_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W79-SELECT-2026-08-23';
if (
  ...
  throw new Error('Wave 79 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave79' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave79-distinct-generated-overhead-operator-name-history-strip',
...
const priorCanonicalTitle = 'Operator Track Record and Mineral Development Risk';
const priorCanonicalSlug = 'operator-track-record-and-mineral-development-risk';
if (
  !sourceRow ||
...
if ([priorCanonicalSlug, slug].includes(sourceRow.canonical_slug)) {
  Object.assign(sourceRow, {
    ...
    secondary_keywords: [keyword, 'Texas operator records', 'operator name comparison'],
    ...
    action_reason:
      'The original operator-track-record/development-risk identity overlapped the live valuation, development-context, public-record, DCF, decline-curve, market-demand, and offset-record corpus. The approved replacement owns only a dated, source-preserving log of operator or payor names displayed in identified records. It uses only same displayed name, different displayed name, or unavailable and stops before corporate-identity, responsibility, property-connection, title, payment, development, valuation, offer, or transaction conclusions. Release remains controlled by the signed batch, matching evidence, deployment, and live verification.',
    ...
    dedupe_evidence: {
      ...
      nearest_same_cluster_slug: 'how-title-defects-change-mineral-rights-offer',
      nearest_same_cluster_title_token_jaccard: 0.3333,
      review_status: 'exact_and_fuzzy_title_check_pass',
    },
    exact_slug_unique: true,
    exact_title_unique: true,
    nearest_same_cluster_slug: 'how-title-defects-change-mineral-rights-offer',
    cannibalization_score: 0.3333,
...
if (
  ...
  throw new Error('Wave 79 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
const articleSha = sha256(articleBytes);
ledger.identity_registry ??= {};
const wave79Rekey = ledger.identity_registry.wave79_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: priorCanonicalSlug,
  prior_source_handle:
    'factory-taxonomy-synthesis:valuation:operator-quality-mineral-development-risk',
...
if (wave79Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 79 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave79_rekey = wave79Rekey;
wave79Rekey.draft_selection_decision_sha256 ??= wave79Rekey.selection_decision_sha256;
wave79Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave79Rekey.release_candidate_article_sha256 = articleSha;
wave79Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The original operator-track-record/development-risk identity was rejected because it overlaps the live valuation, development-context, public-record, DCF, decline-curve, market-demand, and offset-record corpus. The admitted replacement owns only a dated displayed-name log across identified public and owner records.',
    'Current official RRC sources support only bounded descriptions of research-query limitations and displayed wellbore, organization, and drilling-permit name and identifier fields. None establishes corporate relationships, operator or payment responsibility, private title, ownership, property connection, development, appraisal, value, offer analysis, or transaction outcome.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W79-SELECT-2026-08-23; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 79 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave79_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave79_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave79_selection_decision_sha256 = sha256(decisionBytes);
