...
const keyword = 'Factors Affecting Mineral Rights Value';
const inlineFilename = 'factors-affecting-mineral-rights-value';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave25-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave25-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave25-creative-qa/${slug}`),
};

const generationPrompts = {
  hero:
    'Photorealistic mineral-owner decision workspace with an owner, underwriter, mapped tract, abstract production evidence, four unlabeled option cards, a dark exact-title panel, and no readable text outside the exact article title.',
  inline:
    'Compositionally distinct overhead evidence board with mapped ownership layers, an abstract decline curve, document forms, a branching option path, an exact-keyword navy band, and no readable text outside the exact keyword.',
};
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave25-ocr-'));
  try {
