...
const slug = 'the-hidden-traps-in-selling-mineral-rights';
const programRowId = 'MRX1000-0116';
const title = 'The Hidden Traps in Selling Mineral Rights';
const inlineKeyword = 'Common Mistakes Selling Mineral Rights';
const articlePath = `src/content/posts/${slug}.mdx`;
const creativePath = `artifacts/mrx1000-wave36-creative-qa/${slug}/creative-manifest.json`;
...
if (!row || row.slug !== slug || row.title !== title) {
  throw new Error('Wave 36 batch identity is missing or drifted');
}
...
function sourceScope(source) {
  const scopes = new Map([
    ...
    [
      ...
      [
        'In a different consumer-transaction context, the FTC advises reading a completed contract, avoiding blank spaces, resisting pressure, and retaining signed copies.',
        'The article cites this source only as general document-control discipline. It is not mineral-sale law, does not interpret a mineral agreement, and does not establish any owner’s rights or remedies.',
      ],
    ],
  ]);
...
if (
  ...
  throw new Error(
    'Wave 36 review inputs do not satisfy identity, article-depth, source, or creative gates',
  );
}
...
writeArtifact('editorial', `${programRowId}-${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave36_editorial',
  review_run_id: `mrx1000-wave36-editorial-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'editorial',
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; the answer-first article contains ${wordCount} body tokens and five FAQs.`,
    'The article owns the quiet clause-trigger and cross-document dependency job: it separates headline terms from operative definitions, exhibits, diligence rights, price adjustments, exclusivity, deed and payment sequencing, remedies, and owner net, then supplies a trap register, stop rules, and professional handoffs.',
...
writeArtifact('factual_citation', `${slug}.review.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave36_factual',
  review_run_id: `mrx1000-wave36-factual-${reviewedAt.replace(/[-:]/g, '')}`,
  ...
  findings: [
    ...
    'The article uses no fabricated owners, buyers, contracts, clauses, deeds, prices, acreage, ownership decimals, tax results, offers, testimonials, or owner-specific title, legal, tax, accounting, appraisal, investment, or transaction conclusions.',
  ],
  checks: [
...
writeArtifact('compliance', `${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave36_compliance',
  review_run_id: `mrx1000-wave36-compliance-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'compliance',
  expected_hero_sha256: heroSha,
  expected_inline_sha256: inlineSha,
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; hero/share and inline image hashes are separately locked.`,
    'The article states educational and professional boundaries, discloses MRX’s potential economic interest, separates headline terms from the complete document package and expected owner net, supplies explicit stop conditions, labels MRX review as directional, and routes unresolved title, legal, tax, accounting, appraisal, investment, and transaction questions without making owner-specific conclusions.',
    'Image text is limited to the exact article title and canonical keyword and adds no owner data, buyer endorsement, contract interpretation, deed, price, acreage, decimal, formula, appraisal result, tax outcome, or transaction conclusion.',
  ],
  checks: [
    'complete_file_sha256_match',
...
console.log(
  `Built three hash-locked Wave 36 review artifacts with ${sources.length} live source checks.`,
);
