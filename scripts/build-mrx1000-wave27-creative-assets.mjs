...
const slug = 'how-much-are-mineral-rights-worth-checklist-without-obligation';
const title = 'How Much Are Mineral Rights Worth: Checklist Without Obligation';
const keyword = 'how much are mineral rights worth';
const inlineFilename = 'how-much-are-mineral-rights-worth';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave27-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave27-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave27-creative-qa/${slug}`),
};

const generationPrompts = {
  hero:
    'Photorealistic mineral owner reviewing a concise scope-and-disclosure page beside an empty signature folder, muted phone, and two open choice paths, with a dark exact-title panel and no readable words, letters, or numbers outside the exact article title.',
  inline:
    'Compositionally distinct no-people horizontal pathway with privacy, visibility, pause, and two equally open exit-choice icons, an exact-keyword navy band, and no readable words, letters, or numbers outside the exact keyword.',
};
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave27-ocr-'));
  try {
