...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 73 admission argument(s): ${unknownArgs.join(', ')}`);
}

const slug = 'robertson-county-mineral-rights-public-record-locator';
const title = 'How to Build a Robertson County Mineral Rights Public-Record Locator';
const primaryKeyword = 'Robertson County mineral rights public records';
const keyword = 'Robertson County mineral rights public records';
const heroFilename = 'how-to-build-a-robertson-county-mineral-rights-public-record-locator';
const heroAlt =
  'Owner and researcher organize a locator beside “How to Build a Robertson County Mineral Rights Public-Record Locator”.';
const inlineAlt =
  'Three separate public-record source groups surround a blank locator above “Robertson County mineral rights public records”.';
const programRowId = 'MRX1000-1001';
const selectionRank = 153;
...
const decisionRelativePath = 'docs/governance/mrx1000-wave73-selection-decision-2026-08-22.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave73-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave73_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W73-SELECT-2026-08-22';
if (
  ...
  throw new Error('Wave 73 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave73' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave73-distinct-generated-overhead-public-record-locator',
...
const priorCanonicalTitle =
  'Robertson County, Texas Mineral Rights Value: Guide For Mineral Owners';
const priorCanonicalSlug = 'robertson-county-texas-mineral-rights-value-guide-for-mineral-owners';
if (
  !sourceRow ||
...
if (sourceRow.canonical_slug === priorCanonicalSlug) {
  Object.assign(sourceRow, {
    ...
    pillar_url: '/mineral-rights/texas/',
    ...
    action_reason:
      'The planning identity was a generic Robertson County mineral-value guide that would duplicate the valuation corpus and require unsupported local value claims. The approved replacement owns only a county-specific, source-linked public-record locator with five evidence fields and lead located, no result located, or needs professional review labels. It cannot determine title, ownership, authority, acreage, lease meaning, legal effect, production potential, value, offer quality, or whether to transact. Release remains controlled by the signed batch, matching evidence, deployment, and live verification.',
    ...
    dedupe_evidence: {
      ...
      nearest_same_cluster_title_token_jaccard: 0.2,
...
if (
  ...
  throw new Error('Wave 73 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
const articleSha = sha256(articleBytes);
ledger.identity_registry ??= {};
const wave73Rekey = ledger.identity_registry.wave73_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: priorCanonicalSlug,
  prior_source_handle: 'factory-queue:MRX-AEO-08641',
...
if (wave73Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 73 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave73_rekey = wave73Rekey;
wave73Rekey.draft_selection_decision_sha256 ??= wave73Rekey.selection_decision_sha256;
wave73Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave73Rekey.release_candidate_article_sha256 = articleSha;
wave73Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The original identity was rejected because it was a repetitive county mineral-value guide and would require unsupported local valuation claims. The admitted replacement owns only a Robertson County public-record lead locator with five evidence fields and controlled lead located, no result located, or needs professional review labels.',
    'Robertson County Clerk, Texas General Land Office, and Railroad Commission sources are used only to locate and preserve source-specific public-record leads. None establishes title, ownership, authority, acreage, lease meaning, legal effect, production potential, value, offer quality, or a transaction decision.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W73-SELECT-2026-08-22; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 73 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave73_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave73_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave73_selection_decision_sha256 = sha256(decisionBytes);
