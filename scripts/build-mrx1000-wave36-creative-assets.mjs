...
const keyword = 'Common Mistakes Selling Mineral Rights';
const inlineFilename = 'common-mistakes-selling-mineral-rights';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave36-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave36-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave36-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Photorealistic mineral owner reviewing a property map and agreement stack, with a dark navy title-safe panel on the right and no readable words, letters, numbers, prices, percentages, formulas, signatures, logos, or seals.',
  inline:
    'Compositionally distinct no-people overhead contract-trigger map with documents, calendar, scale, deed envelope, shield, and magnifier, plus a lower keyword-safe field and no readable words, letters, numbers, prices, percentages, formulas, signatures, logos, or seals.',
};
...
function heroTypography() {
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      <style>
        .title { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 48px; font-weight: 700; letter-spacing: -1px; }
        .rule { fill: #d79a2b; }
      </style>
      <rect class="rule" x="675" y="180" width="72" height="5" rx="2.5" />
      <text class="title" x="675" y="245">
        <tspan x="675" dy="0">The Hidden Traps</tspan>
        <tspan x="675" dy="58">in Selling</tspan>
        <tspan x="675" dy="58">Mineral Rights</tspan>
      </text>
    </svg>
  `);
}
...
function inlineTypography() {
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect class="rule" x="533" y="466" width="134" height="5" rx="2.5" />
      <text class="keyword" x="600" y="530">
        <tspan x="600" dy="0">Common Mistakes</tspan>
        <tspan x="600" dy="60">Selling Mineral Rights</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave36-ocr-'));
  try {
