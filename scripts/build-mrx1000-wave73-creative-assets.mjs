...
const inlineFilename = 'robertson-county-mineral-rights-public-records';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave73-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave73-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave73-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Premium photorealistic eye-level records workspace with a Texas mineral owner and neutral researcher on the right organizing a blank source-linked locator card; quiet dark field on the left; no readable records, names, property identifiers, values, offers, official seals, logos, conclusions, or success claims.',
  inline:
    'Materially distinct people-free strict top-down public-record locator workspace with three separated blank source groups around a blank five-column grid above a deep-navy keyword field; no readable records, names, identifiers, values, official seals, logos, title conclusions, or ownership claims.',
};
...
function heroTypography() {
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <style>
        .title { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 37px; font-weight: 700; letter-spacing: -0.35px; }
        .rule { fill: #d79a2b; }
      </style>
      <rect x="0" y="0" width="715" height="630" fill="url(#navy)" />
      <rect class="rule" x="46" y="110" width="82" height="5" rx="2.5" />
      <text class="title" x="46" y="181">
        <tspan x="46" dy="0">How to Build a</tspan>
        <tspan x="46" dy="57">Robertson County</tspan>
        <tspan x="46" dy="57">Mineral Rights</tspan>
        <tspan x="46" dy="57">Public-Record Locator</tspan>
      </text>
    </svg>
  `);
}
...
function inlineTypography() {
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect class="rule" x="300" y="472" width="600" height="4" rx="2" />
      <text class="keyword" x="600" y="540">
        <tspan x="600" dy="0">Robertson County mineral</tspan>
        <tspan x="600" dy="54">rights public records</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave73-ocr-'));
  try {
