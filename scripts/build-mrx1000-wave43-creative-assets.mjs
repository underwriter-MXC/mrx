...
const keyword = "How Do I Ensure I'm Receiving a Fair Valuation for My Mineral Rights?";
const inlineFilename = 'how-do-i-ensure-im-receiving-a-fair-valuation-for-my-mineral-rights';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave43-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave43-creative-sources/${slug}-inline-base.png`),
  hero: join(root, 'public/assets/articles/hero/understanding-mineral-rights-how-to-validate-your-assets-true-worth.webp'),
  inline: join(root, `public/assets/articles/inline/${slug}/${inlineFilename}.webp`),
  qa: join(root, `artifacts/mrx1000-wave43-creative-qa/${slug}`),
};

const generationPrompts = {
  hero:
    'Photorealistic mineral owner and analyst validating a mineral-rights range from blank source records, with a title-safe navy field and no readable text, letters, numbers, logos, seals, signatures, private information, prices, formulas, offers, appraisal conclusions, legal conclusions, or ownership conclusions.',
  inline:
    'Compositionally distinct overhead valuation-reperformance workbench with blank source cards, review board, folders, ruler, magnifying glass, mineral core, and a phrase-safe navy panel, with no readable text, letters, numbers, logos, seals, signatures, private information, prices, formulas, offers, appraisal conclusions, legal conclusions, or ownership conclusions.',
};
...
function heroTypography() {
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      <style>
        .title { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 38px; font-weight: 700; letter-spacing: -0.45px; }
        .rule { fill: #d79a2b; }
      </style>
      <rect class="rule" x="52" y="176" width="72" height="5" rx="2.5" />
      <text class="title" x="52" y="230">
        <tspan x="52" dy="0">Understanding</tspan>
        <tspan x="52" dy="48">Mineral Rights:</tspan>
        <tspan x="52" dy="48">How to Validate Your</tspan>
        <tspan x="52" dy="48">Asset’s True Worth</tspan>
      </text>
    </svg>
  `);
}
...
function inlineTypography() {
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      <style>
        .keyword { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 44px; font-weight: 700; text-anchor: middle; letter-spacing: -0.45px; }
        .rule { fill: #d79a2b; }
      </style>
      <rect class="rule" x="530" y="82" width="140" height="5" rx="2.5" />
      <text class="keyword" x="600" y="138">
        <tspan x="600" dy="0">How Do I Ensure I'm</tspan>
        <tspan x="600" dy="52">Receiving a Fair Valuation</tspan>
        <tspan x="600" dy="52">for My Mineral Rights?</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  if (basename(paths.hero, '.webp') !== 'understanding-mineral-rights-how-to-validate-your-assets-true-worth') {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave43-ocr-'));
  try {
    ...
    const manifest = {
      ...
      article: {
        ...
        hero: {
          ...
          public_path: '/assets/articles/hero/understanding-mineral-rights-how-to-validate-your-assets-true-worth.webp',
