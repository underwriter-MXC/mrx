...
const primaryKeyword = 'Why Is My Mineral Rights Valuation Range So Wide?';
const keyword = 'What Makes a Mineral Rights Valuation Range Wider or Narrower?';
const heroAlt =
...
const heroSha256 = '127dbe4851bf1fadca87cbf97114f8eeb90c92dddf7020ec62508f7e03290e2d';
const inlineSha256 = 'b286d5cb03a0f5336d8a4c142dabce4cb0eb7f1a223d0b9d9ca192806a9a67c5';
const programRowId = 'MRX1000-0138';
const selectionRank = 138;
...
const decisionRelativePath = 'docs/governance/mrx1000-wave58-selection-decision-2026-08-14.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave58-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave58_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W58-SELECT-2026-08-14';
if (
  ...
  throw new Error('Wave 58 selection decision remains draft-only and is not publication authority');
}
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave58' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave58-distinct-generated-range-width-map-data-tile-audit',
...
if (
  ...
  throw new Error('Wave 58 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
...
const wave58Rekey = ledger.identity_registry.wave58_rekey ?? {
  program_row_id: programRowId,
  prior_searchatlas_handle:
    'searchatlas-topical-map:256286:title:989994f1-ed08-46d0-9e9b-cdba1eaa2ba3',
...
if (wave58Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 58 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave58_rekey = wave58Rekey;
wave58Rekey.draft_selection_decision_sha256 ??= wave58Rekey.selection_decision_sha256;
wave58Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave58Rekey.release_candidate_article_sha256 = articleSha;
wave58Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The planning-only identity was re-keyed after its primary keyword collided exactly with Article 135. The admitted article owns a distinct range-width audit: it traces endpoint differences to property scope, production and payment reconciliation, producing-cash-flow assumptions, conditional development, benchmark and property bridges, and transaction scope; it records narrowing evidence, reset rules, and stop conditions without duplicating significance screening, local range assembly, intake, full-model validation, or post-range decision pages.',
    'Continuous quality-gated admission under D-2026-0804-16 and MRX1000-W58-SELECT-2026-08-14; no numerical cap or elapsed-time gate applies.',
...
batch.decision_authority.wave58_selection_decision_id = 'MRX1000-W58-SELECT-2026-08-14';
batch.decision_authority.wave58_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave58_selection_decision_sha256 = sha256(decisionBytes);
