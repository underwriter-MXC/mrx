...
process.env.MRX_SOURCE_SCOPES_JSON = JSON.stringify([
  [
    ...
    [
      'The official RRC directory identifies the Oil Proration Query as a named public route and supplies bounded query-system context.',
      'The article records the named route, access time, and published context without treating a query result as complete, legally controlling, connected to private property, or sufficient for a downstream conclusion.',
    ],
  ],
  [
    'https://www.rrc.texas.gov/resource-center/research/research-queries/about-oil-gas-data-queries/',
    [
      'The official Oil and Gas Data Queries page supplies bounded context for the RRC query environment.',
  ...
  [
    ...
    [
      'The official Oil Proration Schedule Query entry displays the separately named route and current criteria labels for one query attempt.',
  ...
  [
    'https://webapps2.rrc.texas.gov/EWA/help/Oil_Proration_TOC.html',
    [
      'The official Oil Proration contents page identifies the query-specific help path and its named sections.',
      'The article uses that path only as a retained source reference and does not treat it as evidence of an individual lease, operator, field, or production conclusion.',
    ],
  ],
  [
    'https://webapps2.rrc.texas.gov/EWA/help/Oil_Proration_HowTo.html',
    [
      'The official help page supplies bounded district, lease, field, operator, county, and navigation instructions.',
      'The article preserves the separate retrieval steps and does not interpret a selected district, lease, field, operator, county, or result.',
    ],
  ],
  [
    'https://webapps2.rrc.texas.gov/EWA/help/OilProration_About.html',
    [
      'The official about page supplies bounded Oil Proration query context.',
      'The article uses that context only to distinguish source labels and retained references from any conclusion about records, operations, production, value, or transactions.',
    ],
  ],
]);
process.env.MRX_EDITORIAL_FINDINGS_JSON = JSON.stringify([
  'The article replaces a colliding valuation explainer with one administrative job: preserve a reproducible provenance trail for a single official RRC Oil Proration Schedule Query attempt through its entry route, criteria snapshot, visible instructions, navigation steps, retained reference, bounded status, and neutral next question.',
...
process.env.MRX_FACTUAL_FINDINGS_JSON = JSON.stringify([
  'Claims remain bounded to current official RRC routes, source-displayed Oil Proration Schedule Query criteria and instructions, separate route identity, query-help context, and published limitations. None of the sources is used to establish property connection, title, ownership, acreage, lease effect, payment responsibility, entitlement, compliance, environmental condition, operating status, production, value, or a transaction result.',
...
process.env.MRX_COMPLIANCE_CHECKS_JSON = JSON.stringify([
  ...
  'texas_rrc_oil_proration_query_retrieval_no_property_connection_title_ownership_acreage_lease_payment_entitlement_compliance_environmental_operating_status_production_value_offer_or_transaction_conclusion_boundary_pass',
