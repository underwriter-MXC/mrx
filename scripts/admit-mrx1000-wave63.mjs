...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 63 admission argument(s): ${unknownArgs.join(', ')}`);
}

const slug = 'what-a-mineral-rights-assessment-does-and-does-not-tell-you';
const title = 'What a Mineral Rights Assessment Does and Does Not Tell You';
const primaryKeyword = 'what does a mineral rights assessment include';
const keyword = 'Mineral Rights Assessment Scope';
const heroFilename = 'what-a-mineral-rights-assessment-does-and-does-not-tell-you';
const heroAlt =
  'An analyst reviews a packet beside “What a Mineral Rights Assessment Does and Does Not Tell You”.';
const inlineAlt =
  'A bounded document review appears above “Mineral Rights Assessment Scope”.';
const programRowId = 'MRX1000-0143';
const selectionRank = 143;
...
const decisionRelativePath = 'docs/governance/mrx1000-wave63-selection-decision-2026-08-14.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave63-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave63_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W63-SELECT-2026-08-14';
if (
  ...
  throw new Error('Wave 63 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave63' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave63-distinct-generated-top-down-assessment-boundary',
...
const priorCanonicalTitle = 'What Influences the Suggested Price for Your Mineral Rights Assessment?';
const priorCanonicalSlug = 'what-influences-the-suggested-price-for-your-mineral-rights-assessment';
if (
  !sourceRow ||
...
if (sourceRow.canonical_slug === priorCanonicalSlug) {
  ...
  action_reason:
    'The replacement scope article was selected after duplicate-intent rejection and remains controlled by the signed batch, matching evidence, deployment, and live verification.',
  ...
  dedupe_evidence: {
    ...
    nearest_same_cluster_title_token_jaccard: 0.1818,
...
if (
  ...
  throw new Error('Wave 63 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
const articleSha = sha256(articleBytes);
ledger.identity_registry ??= {};
const wave63Rekey = ledger.identity_registry.wave63_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: priorCanonicalSlug,
  prior_searchatlas_handle:
    'searchatlas-topical-map:256286:title:28ed4757-a951-4d5a-ac82-0172958e84a6',
...
if (wave63Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 63 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave63_rekey = wave63Rekey;
wave63Rekey.draft_selection_decision_sha256 ??= wave63Rekey.selection_decision_sha256;
wave63Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave63Rekey.release_candidate_article_sha256 = articleSha;
wave63Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The original suggested-price-factor identity was rejected because its exact keyword is already owned by MRX1000-0115 and MRX1000-0130, with related factor-and-range coverage assigned elsewhere. The admitted article owns only the scope and limitations of a directional assessment deliverable.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W63-SELECT-2026-08-14; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 63 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave63_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave63_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave63_selection_decision_sha256 = sha256(decisionBytes);
