...
const slug = 'understanding-the-true-value-of-your-mineral-interests';
const title = 'Understanding the True Value of Your Mineral Interests';
const keyword = 'What to Know Before Selling Mineral Rights';
const inlineFilename =
  'what-to-know-before-selling-mineral-rights';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave53-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave53-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave53-creative-qa/${slug}`),
};

const generationPrompts = {
  hero:
    'No-people mineral-interest evidence desk with layered value lenses, blank proposal envelope, mineral core, Texas map, blank instrument, and distant pumpjack beside a deep-navy exact-title field; no extra readable words, numbers, prices, percentages, dates, signatures, private information, logos, legal conclusions, ownership conclusions, value conclusions, buyer claims, or success claims.',
  inline:
    'Compositionally distinct four-lane tactile value-reconciliation bridge showing evidence, written proposal, blank net record, and decision marker converging on a blank decision record below a navy exact-keyword field; no extra readable words, numbers, prices, percentages, dates, signatures, private information, logos, legal conclusions, ownership conclusions, value conclusions, buyer claims, or success claims.',
};
...
function heroTypography() {
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <style>
        .title { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 43px; font-weight: 700; letter-spacing: -0.6px; }
        .rule { fill: #d79a2b; }
      </style>
      <rect x="0" y="0" width="610" height="630" fill="url(#navy)" />
      <rect class="rule" x="46" y="104" width="82" height="5" rx="2.5" />
      <text class="title" x="46" y="201">
        <tspan x="46" dy="0">Understanding the</tspan>
        <tspan x="46" dy="58">True Value of Your</tspan>
        <tspan x="46" dy="58">Mineral Interests</tspan>
      </text>
    </svg>
  `);
}
...
function inlineTypography() {
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <style>
        .keyword { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 47px; font-weight: 700; text-anchor: middle; letter-spacing: -0.35px; }
        .rule { fill: #d79a2b; }
      </style>
      <rect x="0" y="0" width="1200" height="250" fill="url(#navy)" />
      <rect class="rule" x="0" y="246" width="1200" height="4" />
      <text class="keyword" x="600" y="91">
        <tspan x="600" dy="0">What to Know Before</tspan>
        <tspan x="600" dy="59">Selling Mineral Rights</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave53-ocr-'));
  try {
