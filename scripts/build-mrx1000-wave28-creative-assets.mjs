...
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave28-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave28-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave28-creative-qa/${slug}`),
};

const generationPrompts = {
  hero:
    'Photorealistic mineral owner and underwriter reviewing a mapped tract, blank evidence sheets, an unlabeled production curve, and a mark-only analog clock, with a dark exact-title panel and no readable words, letters, or numbers outside the exact article title.',
  inline:
    'Compositionally distinct no-people horizontal freshness chain with map, blank statement, unlabeled curve, permit, refresh, verification, and open-question icons, an exact-keyword navy band, and no readable words, letters, or numbers outside the exact keyword.',
};
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave28-ocr-'));
  try {
