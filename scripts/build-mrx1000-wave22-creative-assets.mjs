...
const title = 'Five Key Indicators That Show Your Mineral Rights Are Ready for Evaluation';
const keyword = 'How Do I Know if My Mineral Rights Qualify for Evaluation?';
const inlineFilename = 'how-do-i-know-if-my-mineral-rights-qualify-for-evaluation';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave22-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave22-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave22-creative-qa/${slug}`),
};

const generationPrompts = {
  hero:
    'Photorealistic evidence-review desk with five unnumbered amber markers, an exact-title navy panel, and no readable text outside the exact article title.',
  inline:
    'Compositionally distinct evidence wall with five abstract evidence cards, an exact-keyword navy band, and no readable text outside the exact question.',
};
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave22-ocr-'));
  try {
