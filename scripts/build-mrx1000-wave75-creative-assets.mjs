...
const inlineFilename = 'mineral-rights-ai-source-trace';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave75-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave75-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave75-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Premium photorealistic side-angle human-review scene with one calm reviewer, tract map, source records, and three completely blank status cards on the right; quiet dark field on the left; no readable words, names, identifiers, signatures, forms, logos, promises, or fake document text.',
  inline:
    'Materially distinct people-free strict overhead provenance grid connecting a blank output card to an owner folder, abstract public-record map, unresolved marker, blank access-date strip, and blank review checkboxes above a quiet lower field; no readable words, identifiers, signatures, forms, logos, promises, or fake document text.',
};
  ...
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="title" x="46" y="168">
        <tspan x="46" dy="0">How to Keep an</tspan>
        <tspan x="46" dy="58">AI Source Trace for a</tspan>
        <tspan x="46" dy="58">Mineral Rights Review</tspan>
      </text>
    </svg>
  `);
}
  ...
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="keyword" x="600" y="536">
        <tspan x="600" dy="0">mineral rights AI</tspan>
        <tspan x="600" dy="54">source trace</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave75-ocr-'));
  try {
