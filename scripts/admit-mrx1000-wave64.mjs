...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 64 admission argument(s): ${unknownArgs.join(', ')}`);
}

const slug = 'mineral-rights-sale-decision-log';
const title = 'How to Build a Mineral Rights Sale Decision Log';
const primaryKeyword = 'mineral rights sale decision log';
const keyword = 'Mineral Rights Sale Decision Log';
const heroFilename = 'how-to-build-a-mineral-rights-sale-decision-log';
const heroAlt =
  'A mineral owner reviews an organized folder beside “How to Build a Mineral Rights Sale Decision Log”.';
const inlineAlt =
  'An open blank ledger and document tools appear above “Mineral Rights Sale Decision Log”.';
const programRowId = 'MRX1000-0144';
const selectionRank = 144;
...
const decisionRelativePath = 'docs/governance/mrx1000-wave64-selection-decision-2026-08-14.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave64-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave64_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W64-SELECT-2026-08-14';
if (
  ...
  throw new Error('Wave 64 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave64' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave64-distinct-generated-top-down-decision-log',
...
const priorCanonicalTitle = 'What Sellers Get Wrong About Mineral Rights';
const priorCanonicalSlug = 'what-sellers-get-wrong-about-mineral-rights';
if (
  !sourceRow ||
...
if (sourceRow.canonical_slug === priorCanonicalSlug) {
  ...
  action_reason:
    'The replacement decision-log article was selected after exact-keyword and content-job collision rejection and remains controlled by the signed batch, matching evidence, deployment, and live verification.',
  ...
  dedupe_evidence: {
    ...
    nearest_same_cluster_slug: 'top-five-missteps-to-avoid-for-a-successful-mineral-rights-sale',
...
if (
  ...
  throw new Error('Wave 64 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
const articleSha = sha256(articleBytes);
ledger.identity_registry ??= {};
const wave64Rekey = ledger.identity_registry.wave64_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: priorCanonicalSlug,
  prior_searchatlas_handle:
    'searchatlas-topical-map:256286:title:97b70929-02ba-4058-9cac-a53f4ed97102',
...
if (wave64Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 64 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave64_rekey = wave64Rekey;
wave64Rekey.draft_selection_decision_sha256 ??= wave64Rekey.selection_decision_sha256;
wave64Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave64Rekey.release_candidate_article_sha256 = articleSha;
wave64Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The original seller-mistakes identity and first pre-signing replacement were rejected because a live article owns the exact primary keyword and another live article owns the same mistake-prevention checklist job. The admitted replacement owns only construction and maintenance of a dated mineral-rights sale decision log.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W64-SELECT-2026-08-14; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 64 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave64_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave64_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave64_selection_decision_sha256 = sha256(decisionBytes);
