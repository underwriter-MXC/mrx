...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 71 admission argument(s): ${unknownArgs.join(', ')}`);
}
...
const keyword = 'how are mineral rights valued';
const heroFilename = 'how-are-mineral-rights-valued';
const heroAlt =
...
const decisionRelativePath = 'docs/governance/mrx1000-wave71-selection-decision-2026-08-22.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave71-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave71_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W71-SELECT-2026-08-22';
if (
  ...
  throw new Error('Wave 71 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave71' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave71-distinct-generated-overhead-valuation-input-to-range-workspace',
...
if (
  !sourceRow ||
  sourceRow.canonical_slug !== slug ||
  sourceRow.canonical_title !== title ||
  sourceRow.source_handle !== `repo:${articleRelativePath}`
) {
  throw new Error('Canonical incumbent identity is missing or drifted');
}
...
Object.assign(sourceRow, {
  ...
  action_reason:
    'The preserved incumbent is materially refreshed in place as the plain-language conceptual primer explaining how an identified economic interest and stated evidence become a directional range. DCF is the primary explanatory model and market evidence is only a reasonableness check. The article excludes the Texas seven-lock workflow, valuation validation and reperformance, range-width auditing, and all owner-specific title, reserve, engineering, appraisal, legal, tax, value, offer, or transaction conclusions. Release remains controlled by the signed batch, matching evidence, deployment, and live verification.',
  ...
  dedupe_evidence: {
    ...
    nearest_same_cluster_slug: 'acreage-contiguity-and-its-role-in-development-potential',
    nearest_same_cluster_title_token_jaccard: 0,
...
if (
  ...
  throw new Error('Wave 71 canonical incumbent or 1,000-row uniqueness proof failed');
}
const articleSha = sha256(articleBytes);
ledger.identity_registry ??= {};
const wave71Refresh = ledger.identity_registry.wave71_refresh ?? {
  program_row_id: programRowId,
  preserved_canonical_slug: slug,
  preserved_source_handle: `repo:${articleRelativePath}`,
  refresh_type: 'in_place_material_refresh_existing_live_canonical_url',
...
if (
  wave71Refresh.program_row_id !== programRowId ||
  wave71Refresh.preserved_canonical_slug !== slug
) {
  throw new Error('Wave 71 in-place refresh provenance is missing or drifted');
}
ledger.identity_registry.wave71_refresh = wave71Refresh;
wave71Refresh.draft_selection_decision_sha256 ??= wave71Refresh.selection_decision_sha256;
wave71Refresh.selection_decision_sha256 = sha256(decisionBytes);
wave71Refresh.release_candidate_article_sha256 = articleSha;
wave71Refresh.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The established title, slug, keyword, and public URL are preserved. The incumbent is materially refreshed in place as a plain-language conceptual primer on identified royalty economics, conditional cash flow, DCF, and a market-evidence reasonableness check.',
    'RRC production data remains operator-reported and subject to reporting lag and revision; EIA decline analysis is general methodological context; TALCB and Comptroller material support only professional and present-value boundaries. No source establishes owner-specific title, reserves, engineering, value, price, offer, or outcome.',
    'Cannibalization is bounded explicitly: the article does not duplicate the Texas seven-lock valuation workflow, valuation validation or reperformance, the range-width audit, or the planned separate Texas-specific valuation primer.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W71-SELECT-2026-08-22; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 71 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave71_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave71_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave71_selection_decision_sha256 = sha256(decisionBytes);
