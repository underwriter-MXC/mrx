...
const titleLines = ['Comparable Mineral Sales:', 'What Makes a Transaction', 'Relevant?'];
const keyword = 'mineral sale comparability checklist';

const paths = {
  heroSource: join(
    root,
    'artifacts/mrx1000-wave14-creative-sources/comparable-mineral-sales-what-makes-a-transaction-relevant-hero-base.png',
  ),
  inlineSource: join(
    root,
    'artifacts/mrx1000-wave14-creative-sources/comparable-mineral-sales-what-makes-a-transaction-relevant-inline-base.png',
  ),
  hero: join(
    root,
    'public/assets/articles/hero/comparable-mineral-sales-what-makes-a-transaction-relevant.webp',
  ),
  inline: join(
    root,
    'public/assets/articles/inline/comparable-mineral-sales-what-makes-a-transaction-relevant/mineral-sale-comparability-checklist.webp',
  ),
  qa: join(
    root,
    'artifacts/mrx1000-wave14-creative-qa/comparable-mineral-sales-what-makes-a-transaction-relevant',
  ),
};

const generationPrompts = {
  hero: 'Photorealistic oblique aerial West Texas landscape with two adjacent tracts, differing well-pad development stages, and calm left-side title space; no text, labels, numbers, logos, documents, or promotional claims.',
  inline:
    'Distinct photorealistic overhead mineral-sale comparison workspace on dark walnut with three blank navy folders, three unlabeled parcel-photo cards, a brass straightedge, a magnifying lens, and lower-right keyword space; no readable text, numbers, prices, logos, seals, contracts, or wealth imagery.',
};
...
function inlineOverlay() {
  return Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="675">
    ...
    <text x="700" y="458" fill="#fffdf8" font-family="Arial,Helvetica,sans-serif" font-size="41" font-weight="700" letter-spacing="-.6" filter="url(#inline-shadow)">
      <tspan x="700" y="458">mineral sale</tspan>
      <tspan x="700" y="519">comparability checklist</tspan>
    </text>
  </svg>`);
}
...
async function main() {
  ...
  if (basename(paths.hero, '.webp') !== 'comparable-mineral-sales-what-makes-a-transaction-relevant') {
    throw new Error('Hero filename does not match the exact-title slug');
  }
  if (basename(paths.inline, '.webp') !== 'mineral-sale-comparability-checklist') {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave14-ocr-'));
  try {
    ...
    const manifest = {
      ...
      article: {
        ...
        hero: {
          ...
          public_path:
            '/assets/articles/hero/comparable-mineral-sales-what-makes-a-transaction-relevant.webp',
        ...
        inline: {
          ...
          public_path:
            '/assets/articles/inline/comparable-mineral-sales-what-makes-a-transaction-relevant/mineral-sale-comparability-checklist.webp',
