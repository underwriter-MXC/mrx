...
const keyword = 'Navigating Mineral Rights Transactions Texas';
const inlineFilename = 'navigating-mineral-rights-transactions-texas';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave47-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave47-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave47-creative-qa/${slug}`),
};

const generationPrompts = {
  hero:
    'Photorealistic Texas mineral owner routing county, regulatory, payor, proposal, and closing evidence at a warm modern desk, with a distant pumpjack and a clean deep-navy title-safe field on the left; no readable words, letters, numbers, prices, formulas, signatures, private information, logos, seals, ownership conclusions, value conclusions, legal conclusions, or tax conclusions.',
  inline:
    'Compositionally distinct no-people overhead six-lane Texas transaction evidence route with nonverbal courthouse, Texas map, statement, scenario, proposal, and closing-record objects plus an upper navy keyword-safe field; no readable words, letters, numbers, prices, formulas, signatures, private information, logos, seals, ownership conclusions, value conclusions, legal conclusions, or tax conclusions.',
};
...
function heroTypography() {
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect class="rule" x="52" y="150" width="78" height="5" rx="2.5" />
      <text class="title" x="52" y="217">
        <tspan x="52" dy="0">Understanding</tspan>
        <tspan x="52" dy="51">Texas Mineral Rights</tspan>
        <tspan x="52" dy="51">Transactions Today</tspan>
      </text>
    </svg>
  `);
}
...
function inlineTypography() {
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      <style>
        .keyword { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 45px; font-weight: 700; text-anchor: middle; letter-spacing: -0.5px; }
        .rule { fill: #d79a2b; }
      </style>
      <rect class="rule" x="532" y="61" width="136" height="5" rx="2.5" />
      <text class="keyword" x="600" y="124">
        <tspan x="600" dy="0">Navigating Mineral Rights</tspan>
        <tspan x="600" dy="54">Transactions Texas</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave47-ocr-'));
  try {
