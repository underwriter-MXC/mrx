...
const slug = 'selling-mineral-rights-timeline-in-2026';
const programRowId = 'MRX1000-0111';
const title = 'Selling Mineral Rights: Timeline In 2026';
const inlineKeyword = 'selling mineral rights';
const articlePath = `src/content/posts/${slug}.mdx`;
const creativePath = `artifacts/mrx1000-wave31-creative-qa/${slug}/creative-manifest.json`;
...
if (!row || row.slug !== slug || row.title !== title) {
  throw new Error('Wave 31 batch identity is missing or drifted');
}
...
function sourceScope(source) {
  const scopes = new Map([
    [
      'https://mineralrightsxchange.com/blog/how-long-does-it-take-to-sell-mineral-rights-in-texas/',
      [
        'The current MRX Texas timeline article explains that no single Texas sale duration applies and identifies ownership records, buyer diligence, contract conditions, curative work, signing, payment, deed delivery, and recording as timing dependencies.',
        'Wave 31 uses that page only as a state-specific companion and keeps its own job nationwide and centered on dependency lanes, completion evidence, critical-path changes, and reforecasting; it does not restate Texas conveyance, recording, or production-data law.',
      ],
    ],
    [
      'https://mineralrightsxchange.com/blog/selling-mineral-rights-selling-process-for-mineral-owners/',
      [
        'The current MRX nationwide seller-control article gives each transaction stage a required input, owner decision, written output, and stop condition and distinguishes signing, funding, delivery, acceptance, and recording.',
        'Wave 31 uses that page as the process-architecture companion while owning the dated planning calendar, controller lanes, parallel-work distinctions, completion evidence, and reset rules; it does not repeat the full stage-control instructions.',
      ],
    ],
    [
      ...
      [
        ...
        'The article uses the methodology only to explain why timeline inputs need source and retrieval dates and why corrected or newer evidence can trigger a reforecast; it does not calculate value, prove title or reserves, guarantee an outcome, or recommend a transaction.',
      ],
    ],
    [
    ...
    [
      ...
      [
        'The current Instructions for Form 1099-S provide transaction-specific rules for reporting sales or exchanges of real estate and address ownership interests, exceptions, reporting persons, and multiple transferors.',
        'The article uses the instructions only to support a dated tax-professional and information-reporting handoff; it does not conclude whether a mineral owner will receive Form 1099-S or how a sale must be reported.',
      ],
    ],
    [
...
if (
  ...
  throw new Error(
    'Wave 31 review inputs do not satisfy identity, article-depth, source, or creative gates',
  );
}
...
writeArtifact('editorial', `${programRowId}-${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave31_editorial',
  review_run_id: `mrx1000-wave31-editorial-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'editorial',
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; the answer-first article contains ${wordCount} body tokens and five FAQs.`,
    'The article owns the nationwide dependency calendar by separating owner, buyer, joint, and third-party lanes; identifying parallel work and the current critical path; requiring completion evidence; and reforecasting dependent dates after material changes.',
...
writeArtifact('factual_citation', `${slug}.review.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave31_factual',
  review_run_id: `mrx1000-wave31-factual-${reviewedAt.replace(/[-:]/g, '')}`,
  ...
  findings: [
    ...
    'MRX claims remain bounded to the published Texas-timeline and nationwide-process companions and current methodology; IRS claims remain bounded to general disposition, basis-record, and Form 1099-S instruction distinctions; the FBI claim remains bounded to independent verification of unexpected account or payment-instruction changes.',
...
writeArtifact('compliance', `${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave31_compliance',
  review_run_id: `mrx1000-wave31-compliance-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'compliance',
  expected_hero_sha256: heroSha,
  expected_inline_sha256: inlineSha,
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; hero/share and inline image hashes are separately locked.`,
    'The article states educational and professional boundaries, discloses MRX’s potential economic interest, separates planning targets from contractual deadlines and transaction states, and routes unresolved title, legal, tax, accounting, valuation, cybersecurity, escrow, payment, recording, and contract questions without making owner-specific conclusions.',
...
console.log(
  `Built three hash-locked Wave 31 review artifacts with ${sources.length} live source checks.`,
);
