...
const inlineFilename = 'post-production-cost-evidence-packet-royalty-records';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave81-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave81-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave81-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Premium photorealistic front-facing royalty-record evidence-packet scene with a closed folio, separate blank statement sheets, a blank code-legend card, blank correspondence card, clips, and an index tab on the right; uninterrupted navy field on the left; no people, maps, wells, rigs, money, values, curves, readable words, codes, figures, identifiers, dates, signatures, seals, logos, conclusions, or fake document text.',
  inline:
    'Materially distinct people-free strict overhead pale records-matching workspace with separate blank statement, deduction-code legend, correspondence, and provenance-matrix papers in a four-part composition above an uninterrupted lower navy field; no maps, rigs, wells, money, curves, property facts, payment conclusions, readable words, codes, figures, identifiers, dates, signatures, seals, logos, or fake document text.',
};
  ...
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="title" x="46" y="168">
        <tspan x="46" dy="0">Post-Production Cost</tspan>
        <tspan x="46" dy="51">Evidence Packet for</tspan>
        <tspan x="46" dy="51">Royalty Records</tspan>
      </text>
    </svg>
  `);
}
  ...
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="keyword" x="600" y="533">
        <tspan x="600" dy="0">post-production cost evidence</tspan>
        <tspan x="600" dy="48">packet royalty records</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave81-ocr-'));
  try {
