...
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave35-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave35-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave35-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Photorealistic mineral owner and underwriter reconciling a map, production curve, royalty records, and an assumption worksheet, with a dark title-safe panel and no readable words, letters, numbers, prices, percentages, formulas, signatures, logos, or seals.',
  inline:
    'Compositionally distinct no-people overhead factor-to-range traceability map with six connected evidence zones for property, ownership, royalties, production, development, and assumptions, plus a lower question-safe field and no readable words, letters, numbers, prices, percentages, formulas, signatures, logos, or seals.',
};
...
function heroTypography() {
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect class="rule" x="72" y="91" width="72" height="5" rx="2.5" />
      <text class="title" x="72" y="151">
        <tspan x="72" dy="0">The Comprehensive</tspan>
        <tspan x="72" dy="53">Guide to Factors</tspan>
        <tspan x="72" dy="53">Impacting Your</tspan>
        <tspan x="72" dy="53">Mineral Rights</tspan>
        <tspan x="72" dy="53">Valuation</tspan>
      </text>
    </svg>
  `);
}
...
function inlineTypography() {
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      <style>
        .keyword { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 39px; font-weight: 700; text-anchor: middle; letter-spacing: -0.5px; }
        .rule { fill: #d79a2b; }
      </style>
      <rect class="rule" x="533" y="432" width="134" height="5" rx="2.5" />
      <text class="keyword" x="600" y="487">
        <tspan x="600" dy="0">What Factors Influence</tspan>
        <tspan x="600" dy="47">the Price You Suggest</tspan>
        <tspan x="600" dy="47">for My Mineral Rights</tspan>
        <tspan x="600" dy="47">Assessment?</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave35-ocr-'));
  try {
