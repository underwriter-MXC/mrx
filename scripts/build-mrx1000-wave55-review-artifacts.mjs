...
const programRowId = 'MRX1000-0135';
const title = 'Understanding the Value of Your Mineral Rights: Essential Factors to Consider';
const inlineKeyword = 'How Do I Know if My Mineral Rights Have Significant Value?';
const articlePath = `src/content/posts/${slug}.mdx`;
const creativePath = `artifacts/mrx1000-wave55-creative-qa/${slug}/creative-manifest.json`;
...
if (!row || row.slug !== slug || row.title !== title) {
  throw new Error('Wave 55 batch identity is missing or drifted');
}
...
function sourceScope(source) {
  const scopes = new Map([
    ...
    [
      ...
      [
        'The Railroad Commission of Texas lists current production and operating data sets, their formats, update cadence, coverage, and data-set descriptions.',
        'The article uses the page only for dated Texas production and operating-data availability; it does not establish private title, an owner decimal, realized royalty price, reserves, future development, or value.',
      ],
    ],
    [
...
if (
  ...
  throw new Error(
    'Wave 55 review inputs do not satisfy identity, article-depth, source, or creative gates',
  );
}
...
writeArtifact('editorial', `${programRowId}-${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave55_editorial',
  review_run_id: `mrx1000-wave55-editorial-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'editorial',
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; the answer-first article contains ${wordCount} body tokens and five FAQs.`,
    'The article owns the mineral-rights significance-screening job. It tests asset identity, sorts observed producing economics, conditional development signals, written transaction evidence, and limitations, distinguishes missing from negative evidence, supplies a one-page screen, and routes the result to a scoped valuation, reconciliation, identity work, or purpose-specific review.',
...
writeArtifact('factual_citation', `${slug}.review.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave55_factual',
  review_run_id: `mrx1000-wave55-factual-${reviewedAt.replace(/[-:]/g, '')}`,
  ...
  findings: [
    ...
    'Claims remain bounded to current RRC production, query-lag, correction, and data-set context; EIA commodity-series context; and current MRX methodology and selling-options scope. The article preserves source purpose and does not turn any source into an owner-specific title, reserves, development, price, value, offer, or decision result.',
...
writeArtifact('compliance', `${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave55_compliance',
  review_run_id: `mrx1000-wave55-compliance-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'compliance',
  expected_hero_sha256: heroSha,
  expected_inline_sha256: inlineSha,
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; hero/share and inline image hashes are separately locked.`,
    'The article states educational and professional boundaries, separates evidence lanes and next-step routes, labels unresolved identity and professional-purpose questions, discloses possible MRX buyer interest, and makes no owner-specific title, value, legal, tax, investment, appraisal, or transaction conclusion.',
...
console.log(
  `Built three hash-locked Wave 55 review artifacts with ${sources.length} live source checks.`,
);
