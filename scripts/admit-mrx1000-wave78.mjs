...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 78 admission argument(s): ${unknownArgs.join(', ')}`);
}
...
const keyword = 'offset activity property connection';
const heroFilename = 'offset-activity-records-building-a-property-connection-cross-check';
const heroAlt =
...
const decisionRelativePath = 'docs/governance/mrx1000-wave78-selection-decision-2026-08-23.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave78-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave78_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W78-SELECT-2026-08-23';
if (
  ...
  throw new Error('Wave 78 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave78' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave78-distinct-generated-overhead-property-connection-matrix',
...
const priorCanonicalTitle = 'Offset Activity as Evidence of Development Potential';
const priorCanonicalSlug = 'offset-activity-as-evidence-of-development-potential';
if (
  !sourceRow ||
...
if ([priorCanonicalSlug, slug].includes(sourceRow.canonical_slug)) {
  Object.assign(sourceRow, {
    ...
    action_reason:
      'The original generic offset-activity/development-potential identity overlapped the live acreage-contiguity, future-location DCF, and valuation-factor corpus. The approved replacement owns only a field-level property-connection matrix for one identified offset record. It preserves source context, compares one field at a time, uses only match, mismatch, or unverified, and stops before development classification, probability, timing, title, ownership, production attribution, payment, valuation, offer, or transaction conclusions. Release remains controlled by the signed batch, matching evidence, deployment, and live verification.',
...
if (
  ...
  throw new Error('Wave 78 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
const articleSha = sha256(articleBytes);
ledger.identity_registry ??= {};
const wave78Rekey = ledger.identity_registry.wave78_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: priorCanonicalSlug,
  prior_source_handle: 'factory-taxonomy-synthesis:valuation:offset-activity-development-potential',
...
if (wave78Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 78 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave78_rekey = wave78Rekey;
wave78Rekey.draft_selection_decision_sha256 ??= wave78Rekey.selection_decision_sha256;
wave78Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave78Rekey.release_candidate_article_sha256 = articleSha;
wave78Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The original generic offset-activity/development-potential identity was rejected because it overlaps the live acreage-contiguity, future-location DCF, and valuation-factor corpus. The admitted replacement owns only a field-level property-connection matrix for one identified offset record.',
    'Current official RRC sources support only bounded descriptions of permit and wellbore identifiers, query scope, GIS limitations, and production-reporting level, lag, update, and revision context. None establishes private title, ownership, tract or depth inclusion, development, production attribution, payment, appraisal, value, offer analysis, or transaction outcome.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W78-SELECT-2026-08-23; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 78 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave78_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave78_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave78_selection_decision_sha256 = sha256(decisionBytes);
