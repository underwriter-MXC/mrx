...
const title = 'Basis Differentials and Mineral Rights Cash Flow';
const titleLines = ['Basis Differentials', 'and Mineral Rights', 'Cash Flow'];
const keyword = 'basis differential royalty statement';

const paths = {
  heroSource: join(
    root,
    'artifacts/mrx1000-wave13-creative-sources/basis-differentials-and-mineral-rights-cash-flow-hero-base.png',
  ),
  inlineSource: join(
    root,
    'artifacts/mrx1000-wave13-creative-sources/basis-differentials-and-mineral-rights-cash-flow-inline-base.png',
  ),
  hero: join(
    root,
    'public/assets/articles/hero/basis-differentials-and-mineral-rights-cash-flow.webp',
  ),
  inline: join(
    root,
    'public/assets/articles/inline/basis-differentials-and-mineral-rights-cash-flow/basis-differential-royalty-statement.webp',
  ),
  qa: join(
    root,
    'artifacts/mrx1000-wave13-creative-qa/basis-differentials-and-mineral-rights-cash-flow',
  ),
};

const generationPrompts = {
  hero: 'Photorealistic oblique aerial West Texas natural-gas infrastructure showing a credible route from a well pad through gathering and processing facilities, with calm left-side title space; no text, labels, numbers, logos, documents, or promotional claims.',
  inline:
    'Distinct photorealistic overhead royalty-reconciliation still life on dark walnut with abstract unreadable statement pages, an unmarked calculator, and three amber blocks connected by a pipeline model, with lower-right keyword space; no readable text, numbers, logos, seals, or wealth imagery.',
};
...
function heroOverlay() {
  ...
  return Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630">
    ...
    <text fill="#fffdf8" font-family="Arial,Helvetica,sans-serif" font-size="55" font-weight="700" letter-spacing="-.8" filter="url(#hero-shadow)">${tspans}</text>
  </svg>`);
}
...
function inlineOverlay() {
  return Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="675">
    ...
    <text x="700" y="458" fill="#fffdf8" font-family="Arial,Helvetica,sans-serif" font-size="41" font-weight="700" letter-spacing="-.6" filter="url(#inline-shadow)">
      <tspan x="700" y="458">basis differential</tspan>
      <tspan x="700" y="519">royalty statement</tspan>
    </text>
  </svg>`);
}
...
async function main() {
  ...
  if (basename(paths.hero, '.webp') !== 'basis-differentials-and-mineral-rights-cash-flow') {
    throw new Error('Hero filename does not match the exact-title slug');
  }
  if (basename(paths.inline, '.webp') !== 'basis-differential-royalty-statement') {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave13-ocr-'));
  try {
    ...
    const manifest = {
      ...
      article: {
        ...
        hero: {
          ...
          public_path:
            '/assets/articles/hero/basis-differentials-and-mineral-rights-cash-flow.webp',
        ...
        inline: {
          ...
          public_path:
            '/assets/articles/inline/basis-differentials-and-mineral-rights-cash-flow/basis-differential-royalty-statement.webp',
