...
const inlineFilename = 'mineral-rights-sale-decision-log';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave64-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave64-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave64-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Calm documentary mineral owner maintaining an unlabeled transaction decision log with blank ruled paper, neutral folders, and subtle map context on the right beside a deep-navy exact-title field; no cash, buyer branding, fake forms, extra readable words, account numbers, dollar figures, dates, signatures, private information, logos, title conclusions, value conclusions, legal conclusions, recommendations, offers, guarantees, or success claims.',
  inline:
    'Compositionally distinct people-free overhead decision-log workspace with an open blank binder, blank tabs, archival envelope, magnifier, neutral parcel-map fragment, and a lower navy exact-phrase field; no cash, buyer branding, fake forms, extra readable words, account numbers, dollar figures, dates, signatures, private information, logos, title conclusions, value conclusions, legal conclusions, recommendations, offers, guarantees, or success claims.',
};
  ...
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="title" x="46" y="205">
        ...
        <tspan x="46" dy="66">Decision Log</tspan>
      </text>
    </svg>
  `);
}
  ...
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="keyword" x="600" y="535">
        <tspan x="600" dy="0">Mineral Rights Sale</tspan>
        <tspan x="600" dy="62">Decision Log</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave64-ocr-'));
  try {
