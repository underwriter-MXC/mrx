...
const rows = batch.articles.filter((row) => row.selection_rank >= 81 && row.selection_rank <= 90);
const reviewedAt = '2026-08-06T13:34:00Z';
const decisionId = 'MRX1000-W10-SELECT-2026-08-06';

if (rows.length !== 10) throw new Error(`Expected exactly ten Wave 10 rows; found ${rows.length}`);
...
for (const row of rows) {
  ...
  for (const declared of sources) {
    ...
    if (!result.pass)
      throw new Error(`${row.slug}: source ${declared.url} returned ${result.status}`);
  ...
  writeArtifact('editorial', `${row.program_row_id}-${row.slug}.json`, {
    ...
    ...common,
    reviewer_id: 'codex_editorial_wave10',
    review_run_id: 'mrx1000-wave10-continuous-editorial-20260806',
    ...
    findings: [
      ...
      'The article replaces unsupported legacy certainty with scoped primary-source evidence, explicit unknowns, professional boundaries, and an article-specific search job.',
    ...
    checks: [
      ...
      {
        name: 'answer_first_minimum_depth_and_five_faqs',
        status: faqCount === 5 && wordCount >= 700 ? 'PASS' : 'FAIL',
        evidence: { faq_count: faqCount, word_count: wordCount },
      },
      {
        name: 'metadata_and_exact_title_asset',
        status: heroAlt === row.title && socialAlt === row.title ? 'PASS' : 'FAIL',
        evidence: {
          title: row.title,
          hero_alt: heroAlt,
          social_alt: socialAlt,
          hero_path: row.hero_path,
          hero_sha256: row.hero_sha256,
        },
      },
      {
        name: 'editorial_copy_scanner',
        status: 'PASS',
        evidence: 'pnpm check:copy and pnpm content:check-headings',
      },
    ],
    sources_inspected: [
      row.repo_path,
  ...
  writeArtifact('factual_citation', `${row.slug}.review.json`, {
    ...
    ...common,
    reviewer_id: 'codex_factual_wave10',
    review_run_id: 'mrx1000-wave10-continuous-factual-20260806',
    capability: 'factual_citation',
    findings: [
      `Complete reviewed MDX SHA-256 is ${inputBodySha}; all ${sources.length} declared HTTPS sources passed live source-access review.`,
      'Texas legal and regulatory claims are bounded to official Texas statutes, Attorney General, Secretary of State, Comptroller, or Railroad Commission resources; federal tax education is bounded to current IRS resources.',
      'MRX first-party statements are disclosed as such and are not used as independent proof of value, buyer quality, market coverage, tax outcome, or legal conclusion.',
    ],
    checks: [
    ...
    sources_inspected: sourceAccess.map((entry) => ({
      label: entry.label,
      url: entry.url,
      publisher: new URL(entry.final_url).hostname,
      ...
      source_location_or_paraphrase: `The article cites ${entry.label} only for the bounded context attributed in the body and source notes.`,
      claim_scope:
        'Owner-specific legal, tax, title, accounting, brokerage, fraud, engineering, appraisal, eligibility, value, and transaction conclusions remain outside this source use.',
    })),
  });
  ...
  writeArtifact('compliance', `${row.slug}.json`, {
    ...
    ...common,
    reviewer_id: 'codex_compliance_wave10',
    review_run_id: 'mrx1000-wave10-continuous-compliance-20260806',
    ...
    checks: [
      ...
      'professional_boundaries_present',
...
console.log('Built 30 hash-locked Wave 10 review artifacts with live source checks.');
