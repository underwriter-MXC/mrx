...
const programRowId = 'MRX1000-0137';
const title = 'Unlocking the Mystery: Key Details for an Accurate Mineral Rights Review Process';
const inlineKeyword = 'What Information Do I Need to Provide for a Mineral Rights Review?';
const articlePath = `src/content/posts/${slug}.mdx`;
const creativePath = `artifacts/mrx1000-wave57-creative-qa/${slug}/creative-manifest.json`;
...
if (!row || row.slug !== slug || row.title !== title) {
  throw new Error('Wave 57 batch identity is missing or drifted');
}
...
function sourceScope(source) {
  const scopes = new Map([
    ...
    [
      ...
      [
        'The current MRX Sell Mineral Rights page separates holding, selling all, and selling a defined portion and identifies written-proposal and process considerations.',
        'The article uses the page only for owner-option and complete-proposal organization; it does not recommend a path or establish a particular agreement’s meaning, fairness, legal effect, or suitability.',
      ],
    ],
  ]);
...
if (
  ...
  throw new Error(
    'Wave 57 review inputs do not satisfy identity, article-depth, source, or creative gates',
  );
}
...
writeArtifact('editorial', `${programRowId}-${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave57_editorial',
  review_run_id: `mrx1000-wave57-editorial-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'editorial',
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; the answer-first article contains ${wordCount} body tokens and five FAQs.`,
    'The article owns the review-intake identity and evidence-usability job. It defines the owner decision and evidence cutoff, prioritizes property and record identifiers, separates ownership, lease, payment, operating, and offer evidence, preserves support-status labels and conflicts, protects sensitive information, and states pause and professional-handoff conditions.',
...
writeArtifact('factual_citation', `${slug}.review.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave57_factual',
  review_run_id: `mrx1000-wave57-factual-${reviewedAt.replace(/[-:]/g, '')}`,
  ...
  findings: [
    ...
    'Claims remain bounded to current RRC royalty-payment, division-order, well-record, query-identity, reporting-lag, and correction context; Texas recording, county-record, estate, and payor-information statutes; and current MRX methodology and selling-options scope. The article preserves each source purpose and does not turn any source into an owner-specific title, authority, legal effect, reserves, future production, value, tax result, offer, or decision result.',
...
writeArtifact('compliance', `${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave57_compliance',
  review_run_id: `mrx1000-wave57-compliance-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'compliance',
  expected_hero_sha256: heroSha,
  expected_inline_sha256: inlineSha,
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; hero/share and inline image hashes are separately locked.`,
    'The article states educational and professional boundaries, separates evidence lanes, labels supported, owner-reported, assumed, conflicted, missing, and professional-review-needed fields, preserves identity conflicts, protects sensitive data, discloses possible MRX buyer interest, and makes no owner-specific title, authority, value, legal, tax, investment, appraisal, or transaction conclusion.',
...
console.log(
  `Built three hash-locked Wave 57 review artifacts with ${sources.length} live source checks.`,
);
