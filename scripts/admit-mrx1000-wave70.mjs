...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 70 admission argument(s): ${unknownArgs.join(', ')}`);
}
...
const keyword = 'Mineral Rights Valuation Input Correction Packet';
const heroFilename = 'how-to-build-a-mineral-rights-valuation-input-correction-packet';
const heroAlt =
...
const decisionRelativePath = 'docs/governance/mrx1000-wave70-selection-decision-2026-08-22.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave70-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave70_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W70-SELECT-2026-08-22';
if (
  ...
  throw new Error('Wave 70 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave70' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave70-distinct-generated-overhead-valuation-input-correction-packet',
...
const priorCanonicalTitle = 'Your Guide to Fine-Tuning Mineral Rights Valuation';
const priorCanonicalSlug = 'your-guide-to-fine-tuning-mineral-rights-valuation';
if (
  !sourceRow ||
...
if (sourceRow.canonical_slug === priorCanonicalSlug) {
  ...
  pillar_url: '/sell-mineral-rights/',
  ...
  action_reason:
    'The planning title promised broad valuation fine-tuning while its keyword targeted selling-process errors, and both jobs collide with the current valuation and seller-mistakes corpus. The approved replacement begins only after a specific factual input discrepancy is already identified and owns a neutral correction-intake packet with controlled workflow statuses. It cannot validate, reperform, recalculate, revise, appraise, or decide title, acreage, decimal interest, value, fairness, or sell-or-hold. Release remains controlled by the signed batch, matching evidence, deployment, and live verification.',
  ...
  dedupe_evidence: {
    ...
    nearest_same_cluster_slug: 'understanding-mineral-rights-valuation-near-you',
    nearest_same_cluster_title_token_jaccard: 0.1667,
    review_status: 'exact_and_fuzzy_title_check_pass',
  },
  exact_slug_unique: true,
  exact_title_unique: true,
  nearest_same_cluster_slug: 'understanding-mineral-rights-valuation-near-you',
  cannibalization_score: 0.1667,
...
if (
  ...
  throw new Error('Wave 70 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
const articleSha = sha256(articleBytes);
ledger.identity_registry ??= {};
const wave70Rekey = ledger.identity_registry.wave70_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: priorCanonicalSlug,
  prior_searchatlas_handle:
    'searchatlas-topical-map:256286:title:7f0a8bee-706d-4725-903d-9898d5f96b5c',
...
if (wave70Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 70 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave70_rekey = wave70Rekey;
wave70Rekey.draft_selection_decision_sha256 ??= wave70Rekey.selection_decision_sha256;
wave70Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave70Rekey.release_candidate_article_sha256 = articleSha;
wave70Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The original identity was rejected because its broad valuation-fine-tuning title and seller-errors keyword point to different intents already owned by the valuation and sale-process corpus. The admitted replacement owns only a neutral correction-intake packet after a specific discrepancy is identified.',
    'EPA and NARA sources are used only as transparent federal administrative analogies for specific correction-request fields and the source/input/output distinction. They do not govern a private mineral-rights review, validate a replacement, create a correction right, or require private retention.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W70-SELECT-2026-08-22; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 70 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave70_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave70_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave70_selection_decision_sha256 = sha256(decisionBytes);
