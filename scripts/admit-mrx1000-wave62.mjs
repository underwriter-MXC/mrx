...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 62 admission argument(s): ${unknownArgs.join(', ')}`);
}
...
const decisionRelativePath = 'docs/governance/mrx1000-wave62-selection-decision-2026-08-14.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave62-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave62_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W62-SELECT-2026-08-14';
if (
  ...
  throw new Error('Wave 62 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave62' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave62-distinct-generated-top-down-unit-calculation',
...
if (
  ...
  ledger.articles.some(
    (row) => row.canonical_slug === 'what-factors-determine-your-mineral-rights-value',
  )
) {
  throw new Error('Wave 62 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
...
const wave62Rekey = ledger.identity_registry.wave62_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: 'what-factors-determine-your-mineral-rights-value',
  prior_searchatlas_handle:
    'searchatlas-topical-map:254654:title:6bbc8e66-751c-4907-bc14-280a309074d3',
...
if (wave62Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 62 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave62_rekey = wave62Rekey;
wave62Rekey.draft_selection_decision_sha256 ??= wave62Rekey.selection_decision_sha256;
wave62Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave62Rekey.release_candidate_article_sha256 = articleSha;
wave62Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The original valuation-factor identity was rejected because it collides with MRX1000-0167, MRX1000-0183, MRX1000-0291, MRX1000-0294, and the current valuation-guide corpus. The admitted article owns only the arithmetic job of dividing one written total offer by the confirmed NMA covered by that same offer.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W62-SELECT-2026-08-14; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 62 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave62_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave62_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave62_selection_decision_sha256 = sha256(decisionBytes);
