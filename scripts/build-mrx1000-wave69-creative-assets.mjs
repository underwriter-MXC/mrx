...
const inlineFilename = 'mineral-rights-offer-correspondence-index';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave69-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave69-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave69-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Premium documentary photograph of one adult mineral owner at an eye-level three-quarter desk view on the right, calmly organizing blank envelopes and two closed navy folders beside a laptop with a blank screen, with warm natural window light and a clean deep-navy exact-title field on the left; no readable correspondence, names, addresses, dates, prices, acreage, signatures, official seals, buyer branding, fake records, legal conclusions, value conclusions, rankings, recommendations, guarantees, or success claims.',
  inline:
    'Compositionally distinct people-free overhead correspondence-index workspace with six blank communication cards, closed navy folders, blank envelopes, neutral connector tabs, and a capped gold pen above a clean deep-navy lower exact-phrase field; no people, hands, screens, readable correspondence, names, addresses, dates, prices, acreage, signatures, official seals, buyer branding, fake records, legal conclusions, value conclusions, rankings, recommendations, guarantees, or success claims.',
};
  ...
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="title" x="46" y="205">
        ...
        <tspan x="46" dy="68">Mineral Rights Offer</tspan>
        <tspan x="46" dy="68">Correspondence Index</tspan>
      </text>
    </svg>
  `);
}
  ...
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="keyword" x="600" y="530">
        ...
        <tspan x="600" dy="54">Correspondence Index</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave69-ocr-'));
  try {
