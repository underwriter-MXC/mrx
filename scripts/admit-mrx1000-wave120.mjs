#!/usr/bin/env node

process.env.MRX_WAVE_NUMBER = '120';
process.env.MRX_ARTICLE_SLUG =
  'texas-rrc-imaged-potential-file-retrieval-provenance-worksheet';
process.env.MRX_ARTICLE_TITLE =
  'Texas RRC Imaged Potential File Retrieval Provenance Worksheet';
process.env.MRX_PRIMARY_KEYWORD = 'Texas RRC potential file retrieval';
process.env.MRX_INLINE_KEYWORD = 'Texas RRC potential file retrieval';
process.env.MRX_HERO_FILENAME =
  'texas-rrc-imaged-potential-file-retrieval-provenance-worksheet';
process.env.MRX_HERO_ALT =
  'A researcher opens a generic archive drawer beside the exact article title.';
process.env.MRX_INLINE_ALT =
  'A people-free overhead potential-file evidence board appears above the exact keyword.';
process.env.MRX_PROGRAM_ROW_ID = 'MRX1000-0321';
process.env.MRX_SELECTION_RANK = '197';
process.env.MRX_DECISION_ID = 'MRX1000-W120-SELECT-2026-08-25';
process.env.MRX_DECISION_PATH =
  'docs/governance/mrx1000-wave120-selection-decision-2026-08-25.md';
process.env.MRX_REVIEWED_BY = 'mrx_compliance-continuous-wave120';
process.env.MRX_PRIOR_TITLE =
  'What to Do When You Have Multiple Offers for Your Mineral Rights';
process.env.MRX_PRIOR_SLUG =
  'what-to-do-when-you-have-multiple-offers-for-your-mineral-rights';
process.env.MRX_PRIOR_SOURCE_HANDLE =
  'repo:src/content/posts/what-to-do-when-you-have-multiple-offers-for-your-mineral-rights.mdx';
process.env.MRX_SECONDARY_KEYWORDS_JSON = JSON.stringify([
  'Texas RRC potential file retrieval',
  'RRC Oil and Gas Imaged Records',
  'Texas potential file provenance worksheet',
  'RRC FILM YR evidence note',
]);
process.env.MRX_NEAREST_SAME_CLUSTER_SLUG =
  'texas-rrc-imaged-well-log-retrieval-provenance-worksheet';
process.env.MRX_CANNIBALIZATION_SCORE = '0.6000';
process.env.MRX_INLINE_VISUAL_VARIANT =
  'wave120-distinct-generated-people-free-overhead-potential-file-evidence-board';
process.env.MRX_PILLAR = 'offer-review';
process.env.MRX_PILLAR_URL = '/offer-review/';
process.env.MRX_CLUSTER = 'offer-review-buyer-comparison-safety';
process.env.MRX_FUNNEL_STAGE = 'consideration';
process.env.MRX_ACTION_REASON =
  'The original multiple-offers identity was rejected for direct overlap and unsupported high-risk claims. The approved replacement owns one authorized manual Texas RRC Imaged Records Oil and Gas Potential profile retrieval provenance record and remains separate from the Well Log profile, adjacent RRC query and record systems, interpretation, offer, and decision tasks. The source is prepared for the exact release-10 build, but publication remains controlled by the signed batch, matching evidence, production deployment, and independent verification.';
process.env.MRX_RISK_REMEDIATION_JSON = JSON.stringify([
  'The original multiple-offers identity is rejected for direct overlap. None of its percentages, invented examples, valuation or legal assertions, buyer-conduct claims, negotiation guidance, promotional language, or service-booking pitch survives.',
  'Current official Texas RRC sources support only Imaged Records and Oil and Gas Potential profile identity, stated record categories and coverage, later electronic-route separation, FILM YR and viewer context, route boundaries, agency-scope limitations, displayed source state, and source-scope limitations. None establishes a completed retrieval, record completeness, well fact, property connection, ownership, title, payment, production, value, offer quality, compliance, legal effect, tax effect, or transaction outcome.',
  'The article claims no completed direct-profile query. Blocked, error, timeout, validation-failure, default, session-termination, unavailable-viewer, inaccessible-profile, or otherwise inconclusive states remain unverified; only a completed empty result attributable to accepted criteria can be not located.',
  'The article prohibits automation, scraping, bulk retrieval, repeated-query workflows, access-control bypass, CAPTCHA handling, workarounds, record interpretation, and substantive inference.',
  'Continuous quality-gated admission under the owner no-approval directives; no owner publication approval applies.',
]);

await import('./admit-mrx1000-wave82.mjs');
