...
const programRowId = 'MRX1000-0121';
const title = 'Understanding Market Demand: How It Influences the Value of Your Mineral Rights';
const inlineKeyword = 'How Does the Market Demand Affect the Price I Can Expect for My Rights?';
const articlePath = `src/content/posts/${slug}.mdx`;
const creativePath = `artifacts/mrx1000-wave41-creative-qa/${slug}/creative-manifest.json`;
...
if (!row || row.slug !== slug || row.title !== title) {
  throw new Error('Wave 41 batch identity is missing or drifted');
}
...
function sourceScope(source) {
  const scopes = new Map([
    ...
    [
      ...
      [
        'The U.S. Energy Information Administration explains that global supply and demand drive crude-oil prices, inventories help balance the market, short-run constraints can contribute to volatility, and price outlooks remain uncertain.',
        'The article uses the page only for the commodity-market demand layer and forecast-uncertainty boundary; it does not convert broad oil-market conditions into a property-specific realized price, value, or offer.',
      ],
    ],
    [
      'https://www.eia.gov/energyexplained/natural-gas/factors-affecting-natural-gas-prices.php',
      [
        'The U.S. Energy Information Administration explains that natural-gas prices respond to market supply and demand, including production, storage, imports and exports, weather, economic conditions, infrastructure, and other fuels.',
        'The article uses the page only for the natural-gas commodity-demand layer and its stated drivers; it does not convert broad or national gas-market conditions into a property-specific realized price, value, or offer.',
      ],
    ],
    [
      ...
      [
        ...
        'The article uses these queries only for dated development and operational context; it does not treat a query result as proof of ownership, tract inclusion, drilling, production, payment, buyer demand, or future development.',
      ],
    ],
    [
      'https://www.rrc.texas.gov/resource-center/research/data-sets-available-for-download/',
      [
        'The Railroad Commission of Texas describes downloadable drilling-permit, wellbore, field, and production-related datasets, their fields, and their update schedules.',
        'The article uses the datasets only to define available dated activity evidence and reporting scope; it does not infer a property-specific commitment, result, value, private transaction price, or buyer mandate.',
      ],
    ],
  ]);
...
if (
  ...
  throw new Error(
    'Wave 41 review inputs do not satisfy identity, article-depth, source, or creative gates',
  );
}
...
writeArtifact('editorial', `${programRowId}-${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave41_editorial',
  review_run_id: `mrx1000-wave41-editorial-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'editorial',
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; the answer-first article contains ${wordCount} body tokens and five FAQs.`,
    'The article owns the three-layer market-demand transmission-audit job: it separates commodity-market demand, tract-connected development activity, and observable acquisition demand, then supplies a thirteen-field dated demand snapshot and stop conditions.',
...
writeArtifact('factual_citation', `${slug}.review.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave41_factual',
  review_run_id: `mrx1000-wave41-factual-${reviewedAt.replace(/[-:]/g, '')}`,
  ...
  findings: [
    ...
    'MRX claims remain bounded to its current methodology and FAQ; commodity-market claims remain bounded to EIA oil and natural-gas explanations; development-activity claims remain bounded to Railroad Commission query and dataset scope and limitations.',
...
writeArtifact('compliance', `${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave41_compliance',
  review_run_id: `mrx1000-wave41-compliance-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'compliance',
  expected_hero_sha256: heroSha,
  expected_inline_sha256: inlineSha,
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; hero/share and inline image hashes are separately locked.`,
    'The article separates commodity-market demand, tract-connected development activity, and acquisition demand rather than making an owner-specific market-value conclusion; discloses MRX’s possible economic interest; dates and scopes each signal; separates public context from property evidence; normalizes buyer terms; separates asset range from transaction scope; and routes professional questions without making owner-specific conclusions.',
    'Image text is limited to the exact article title and canonical question after incidental generated screen, chart, coin, seal, and folder marks were neutralized; the images add no owner data, ownership conclusion, production value, commodity price, acreage, decimal, formula, offer, transaction term, appraisal result, regulatory endorsement, tax outcome, or private information.',
  ],
  checks: [
    'complete_file_sha256_match',
...
console.log(
  `Built three hash-locked Wave 41 review artifacts with ${sources.length} live source checks.`,
);
