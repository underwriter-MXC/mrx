...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 67 admission argument(s): ${unknownArgs.join(', ')}`);
}

const slug = 'how-to-choose-a-family-point-person-for-mineral-rights-inquiries';
const title = 'How to Choose a Family Point Person for Mineral Rights Inquiries';
const primaryKeyword = 'mineral rights family point person';
const keyword = 'One Contact, Shared Information';
const heroFilename = 'how-to-choose-a-family-point-person-for-mineral-rights-inquiries';
const heroAlt =
  'A family chooses one notebook holder beside “How to Choose a Family Point Person for Mineral Rights Inquiries”.';
const inlineAlt =
  'Blank envelopes surround one notebook above “One Contact, Shared Information”.';
const programRowId = 'MRX1000-0147';
const selectionRank = 147;
...
const decisionRelativePath = 'docs/governance/mrx1000-wave67-selection-decision-2026-08-14.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave67-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave67_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W67-SELECT-2026-08-14';
if (
  ...
  throw new Error('Wave 67 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave67' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave67-distinct-generated-family-inquiry-routing-workspace',
...
if (sourceRow.canonical_slug === priorCanonicalSlug) {
  ...
  action_reason:
    'The replacement family-point-person article was selected after the generic selling-process identity collided with live process, timing, guide, mistake, trap, partial-sale, and post-sale jobs. It owns only a non-legal administrative contact protocol after engagement intent is settled and remains controlled by the signed batch, matching evidence, deployment, and live verification.',
  ...
  dedupe_evidence: {
    ...
    nearest_same_cluster_slug: 'how-to-talk-to-your-family-before-you-decide-to-sell-mineral-rights',
...
if (
  ...
  throw new Error('Wave 67 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
const articleSha = sha256(articleBytes);
ledger.identity_registry ??= {};
const wave67Rekey = ledger.identity_registry.wave67_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: priorCanonicalSlug,
  prior_searchatlas_handle:
    'searchatlas-topical-map:254654:title:ca646f5c-c51c-4cc3-ab97-f3d9013c8a2b',
...
if (wave67Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 67 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave67_rekey = wave67Rekey;
wave67Rekey.draft_selection_decision_sha256 ??= wave67Rekey.selection_decision_sha256;
wave67Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave67Rekey.release_candidate_article_sha256 = articleSha;
wave67Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The original generic selling-process identity was rejected because live articles already own seller process, timing, comprehensive-guide, mistake, hidden-trap, partial-sale, and post-sale jobs. A valuation-focused replacement was also rejected for direct corpus collision and source drift. The admitted replacement owns only a non-legal family point-person protocol after the family has already chosen to receive inquiries.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W67-SELECT-2026-08-14; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 67 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave67_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave67_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave67_selection_decision_sha256 = sha256(decisionBytes);
