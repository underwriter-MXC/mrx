...
const inlineFilename = 'mineral-rights-assessment-scope';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave63-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave63-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave63-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Calm documentary mineral-rights analyst reviewing one unlabeled assessment packet, blank evidence summary, and subtle map context on the right beside a deep-navy exact-title field; no cash, buyer branding, fake forms, extra readable words, account numbers, dollar figures, dates, signatures, private information, logos, title conclusions, value conclusions, appraisal conclusions, tax results, legal conclusions, recommendations, offers, guarantees, or success claims.',
  inline:
    'Compositionally distinct people-free overhead assessment-scope boundary with one blank packet, magnifier, neutral map fragment, closed professional folders, and a lower navy exact-phrase field; no cash, buyer branding, fake forms, extra readable words, account numbers, dollar figures, dates, signatures, private information, logos, title conclusions, value conclusions, appraisal conclusions, tax results, legal conclusions, recommendations, offers, guarantees, or success claims.',
};
...
function heroTypography() {
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <style>
        .title { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 45px; font-weight: 700; letter-spacing: -0.35px; }
        .rule { fill: #d79a2b; }
      </style>
      ...
      <text class="title" x="46" y="205">
        <tspan x="46" dy="0">What a Mineral Rights</tspan>
        <tspan x="46" dy="66">Assessment Does and</tspan>
        <tspan x="46" dy="66">Does Not Tell You</tspan>
      </text>
    </svg>
  `);
}
...
function inlineTypography() {
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect class="rule" x="330" y="449" width="540" height="4" rx="2" />
      <text class="keyword" x="600" y="535">
        <tspan x="600" dy="0">Mineral Rights</tspan>
        <tspan x="600" dy="62">Assessment Scope</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave63-ocr-'));
  try {
