...
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave29-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave29-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave29-creative-qa/${slug}`),
};

const generationPrompts = {
  hero:
    'Photorealistic mineral owner and reviewer following an ordered evidence workflow across a mapped tract, blank document folder, unlabeled statement bars, unlabeled production curve, development icons, assumptions sheet, and bounded-range folder, with a dark exact-title panel and no readable words, letters, or numbers outside the exact article title.',
  inline:
    'Compositionally distinct no-people overhead decision-gate board with map, blank folder, unlabeled bars and curve, development icons, assumption sliders, specialist-handoff compass, checkpoints, a central exact-keyword folder, and no readable words, letters, or numbers outside the exact keyword.',
};
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave29-ocr-'));
  try {
