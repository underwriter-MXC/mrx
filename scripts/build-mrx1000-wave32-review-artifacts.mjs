...
const slug = 'selling-mineral-rights-valuation-factors-in-2026';
const programRowId = 'MRX1000-0112';
const title = 'Selling Mineral Rights: Valuation Factors In 2026';
const inlineKeyword = 'selling mineral rights';
const articlePath = `src/content/posts/${slug}.mdx`;
const creativePath = `artifacts/mrx1000-wave32-creative-qa/${slug}/creative-manifest.json`;
...
if (!row || row.slug !== slug || row.title !== title) {
  throw new Error('Wave 32 batch identity is missing or drifted');
}
  ...
  const scopes = new Map([
    [
      ...
      [
        ...
        'The article uses the methodology to preserve the directional-range, scenario, and limitation boundaries while owning the seller-facing factor register and offer bridge; it does not calculate a property-specific value, prove title or reserves, guarantee an outcome, or recommend a transaction.',
      ],
    ],
    [
      'https://mineralrightsxchange.com/blog/key-factors-that-determine-your-mineral-rights-assessment-pricing-range/',
      [
        'The current MRX assessment-range article identifies producing status, production and decline, ownership and acreage, lease and royalty terms, commodity assumptions, operator and development context, title and data confidence, and transaction terms as principal review inputs.',
        'Wave 32 uses that page as the concise factor companion while adding a three-ledger evidence register, source dates, confidence labels, scenario effects, reset triggers, and a factor-by-factor comparison bridge; it does not duplicate the concise factor summary.',
      ],
    ],
    [
      'https://mineralrightsxchange.com/blog/understanding-the-key-factors-influencing-your-mineral-rights-offer-range/',
      [
        'The current MRX offer-range article explains how the exact interest, ownership evidence, production, lease economics, commodity assumptions, development timing, risk, buyer scope, contract conditions, and expected net proceeds can make buyer ranges differ.',
        'Wave 32 uses that page as the detailed range-difference companion while owning the owner-controlled 2026 factor register and bridge; it does not claim that every buyer discloses its strategy, weighting, return requirement, or full underwriting model.',
      ],
    ],
    [
...
if (
  ...
  throw new Error(
    'Wave 32 review inputs do not satisfy identity, article-depth, source, or creative gates',
  );
}
...
writeArtifact('editorial', `${programRowId}-${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave32_editorial',
  review_run_id: `mrx1000-wave32-editorial-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'editorial',
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; the answer-first article contains ${wordCount} body tokens and five FAQs.`,
    'The article owns the seller-facing 2026 factor register by separating asset facts, forecast assumptions, and transaction terms; assigning sources, as-of dates, confidence labels, scenario effects, verification owners, and reset triggers; and bridging ranges or offers factor by factor.',
...
writeArtifact('factual_citation', `${slug}.review.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave32_factual',
  review_run_id: `mrx1000-wave32-factual-${reviewedAt.replace(/[-:]/g, '')}`,
  ...
  findings: [
    ...
    'MRX claims remain bounded to the current methodology and factor companions; EIA claims remain bounded to dated public data series; the SEC claim remains bounded to public-company reserve-reporting convention; the Texas regulator claim remains bounded to operator-reported production data; and IRS claims remain bounded to general disposition and basis-record distinctions.',
...
console.log(
  `Built three hash-locked Wave 32 review artifacts with ${sources.length} live source checks.`,
);
