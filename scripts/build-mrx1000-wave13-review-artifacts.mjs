...
const inlineKeyword = 'basis differential royalty statement';
const articlePath = `src/content/posts/${slug}.mdx`;
const creativePath = `artifacts/mrx1000-wave13-creative-qa/${slug}/creative-manifest.json`;
...
if (!row || row.slug !== slug || row.title !== title) {
  throw new Error('Wave 13 batch identity is missing or drifted');
}
...
function sourceScope(source) {
  const scopes = new Map([
    [
      ...
      [
        'FERC describes Henry Hub as a widely used U.S. natural-gas reference and explains regional hubs and location differentials.',
        'This source supports market-structure education, not an owner-specific realized price, contract interpretation, or payment conclusion.',
      ],
    ],
    [
      'https://www.eia.gov/energyexplained/natural-gas/prices.php',
      [
        'EIA explains natural-gas price components and why prices can differ across locations and customer classes.',
        'This source does not establish the private sales price or royalty treatment for a particular lease.',
      ],
    ],
    [
      'https://www.eia.gov/energyexplained/natural-gas/factors-affecting-natural-gas-prices.php',
      [
        'EIA identifies supply, demand, weather, storage, production, and pipeline constraints as natural-gas price factors.',
        'This source supports general market drivers, not a forecast or a permanent basis assumption.',
      ],
    ],
    [
      'https://www.eia.gov/todayinenergy/detail.php?id=63504',
      [
        'EIA documents differing outcomes at regional natural-gas pricing hubs under local supply, demand, and transportation conditions.',
        'This source does not prove the market point used for the article reader’s property.',
      ],
    ],
    [
      'https://www.eia.gov/todayinenergy/detail.php?id=53919',
      [
        'EIA describes changes in Waha basis associated with Permian production, pipeline capacity, and maintenance.',
        'The example is regional context and is not presented as a current quote or a universal adjustment.',
      ],
    ],
    [
      'https://statutes.capitol.texas.gov/Docs/NR/pdf/NR.91.pdf',
      [
        'Texas Natural Resources Code Section 91.502 lists information that accompanies certain oil and gas production payments.',
        'The article preserves statutory conditions and legal-review boundaries and does not decide lease compliance or a remedy.',
      ],
    ],
    [
      'https://www.rrc.texas.gov/oil-and-gas/research-and-statistics/production-data/',
      [
        'The Railroad Commission describes its operator-reported oil and gas production data resources.',
        'The source provides regulatory context, not an owner decimal, private sales price, or complete royalty audit.',
      ],
    ],
    [
      'https://www.rrc.texas.gov/about-us/faqs/oil-gas-faq/production-data-query-system-faqs/',
      [
        'The Railroad Commission explains lease- and well-level reporting, reporting lag, and later production-data corrections.',
        'The source supports public-data limitations and does not establish an owner-specific payment conclusion.',
      ],
    ],
  ]);
...
if (
  ...
  throw new Error(
    'Wave 13 review inputs do not satisfy identity, depth, source, or creative gates',
  );
}
...
writeArtifact('editorial', `${programRowId}-${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave13_editorial',
  review_run_id: `mrx1000-wave13-editorial-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'editorial',
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; the answer-first article contains ${wordCount} body tokens and five FAQs.`,
    'The benchmark-to-payment bridge separates regional basis, price adjustments, stated deductions, taxes, owner decimal, and net payment without alleging an owner-specific breach.',
...
writeArtifact('factual_citation', `${slug}.review.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave13_factual',
  review_run_id: `mrx1000-wave13-factual-${reviewedAt.replace(/[-:]/g, '')}`,
  ...
  findings: [
    ...
    'Market claims remain bounded to FERC and EIA; Texas statement and production-data claims remain bounded to the statute and Railroad Commission resources.',
    'The article uses no fabricated market price, owner decimal, payment amount, testimonial, or owner-specific legal conclusion.',
  ],
  checks: [
...
writeArtifact('compliance', `${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave13_compliance',
  review_run_id: `mrx1000-wave13-compliance-${reviewedAt.replace(/[-:]/g, '')}`,
...
console.log(
  `Built three hash-locked Wave 13 review artifacts with ${sources.length} live source checks.`,
);
