...
const inlineFilename = 'how-are-mineral-rights-valued';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave71-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave71-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave71-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Premium documentary photograph at eye level of one adult mineral owner and one analyst on the left side of a light table, calmly reviewing blank cash-flow evidence cards with an oil pumpjack visible through the window and a clean deep-navy exact-title field on the right; no readable records, names, addresses, dates, prices, acreage, decimals, numerical results, signatures, official seals, buyer branding, fake evidence, legal conclusions, value conclusions, rankings, recommendations, guarantees, or success claims.',
  inline:
    'Compositionally distinct people-free overhead mineral-rights valuation-input workspace with generic nonnumeric production, royalty-term, forecast-assumption, and market-check papers, closed navy folders, ruler, pencil, and calculator above a clean deep-navy lower exact-keyword field; no people, hands, screens, readable records, names, addresses, dates, prices, acreage, decimals, numerical results, signatures, official seals, buyer branding, fake evidence, legal conclusions, value conclusions, rankings, recommendations, guarantees, or success claims.',
};
...
function heroTypography() {
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <style>
        .title { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 51px; font-weight: 700; letter-spacing: -0.45px; }
        .rule { fill: #d79a2b; }
      </style>
      <rect x="610" y="0" width="590" height="630" fill="url(#navy)" />
      <rect class="rule" x="662" y="148" width="94" height="5" rx="2.5" />
      <text class="title" x="662" y="231">
        <tspan x="662" dy="0">How Are</tspan>
        <tspan x="662" dy="72">Mineral Rights</tspan>
        <tspan x="662" dy="72">Valued?</tspan>
      </text>
    </svg>
  `);
}
...
function inlineTypography() {
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <style>
        .keyword { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 47px; font-weight: 700; text-anchor: middle; letter-spacing: -0.3px; }
        .rule { fill: #d79a2b; }
      </style>
      <rect x="0" y="425" width="1200" height="250" fill="url(#navy)" />
      <rect class="rule" x="330" y="465" width="540" height="4" rx="2" />
      <text class="keyword" x="600" y="538">
        <tspan x="600" dy="0">how are mineral rights</tspan>
        <tspan x="600" dy="58">valued</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave71-ocr-'));
  try {
