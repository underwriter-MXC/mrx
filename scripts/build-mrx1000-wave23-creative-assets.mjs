...
const keyword = 'Free Mineral Rights Valuation Review';
const inlineFilename = 'free-mineral-rights-valuation-review';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave23-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave23-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave23-creative-qa/${slug}`),
};

const generationPrompts = {
  hero:
    'Photorealistic evidence-to-output desk with a dark exact-title panel, mapped tract, production curve, royalty evidence, and no readable text outside the exact article title.',
  inline:
    'Compositionally distinct evidence wall with five abstract review-input cards, an exact-keyword navy band, and no readable text outside the exact keyword.',
};
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave23-ocr-'));
  try {
