...
const slug = 'understanding-texas-mineral-rights-valuation-process';
const programRowId = 'MRX1000-0128';
const title = 'Understanding Texas Mineral Rights Valuation Process';
const inlineKeyword = 'Texas Mineral Rights Valuation Process';
const articlePath = `src/content/posts/${slug}.mdx`;
const creativePath = `artifacts/mrx1000-wave48-creative-qa/${slug}/creative-manifest.json`;
...
if (!row || row.slug !== slug || row.title !== title) {
  throw new Error('Wave 48 batch identity is missing or drifted');
}
...
if (
  ...
  throw new Error(
    'Wave 48 review inputs do not satisfy identity, article-depth, source, or creative gates',
  );
}
...
writeArtifact('editorial', `${programRowId}-${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave48_editorial',
  review_run_id: `mrx1000-wave48-editorial-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'editorial',
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; the answer-first article contains ${wordCount} body tokens and five FAQs.`,
    'The article owns an end-to-end Texas valuation-process audit trail: decision and date, exact interest scope, identifier reconciliation, normalized production and payment evidence, labeled economic assumptions, method fit, sensitivities, output reconciliation, and decision record. It routes factor, method-overview, pathway-selection, provider-routing, and existing-result reperformance jobs to their established pages.',
...
writeArtifact('factual_citation', `${slug}.review.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave48_factual',
  review_run_id: `mrx1000-wave48-factual-${reviewedAt.replace(/[-:]/g, '')}`,
  ...
  findings: [
    ...
    'MRX claims remain bounded to its current FAQ, methodology, How It Works page, and terms; Railroad Commission query and production guidance stays within the agency’s stated operational and informational limits; EIA benchmark data retains its source labels; the Comptroller manual remains confined to its Texas property-tax purpose; and IRS claims remain general value and disposition concepts.',
...
writeArtifact('compliance', `${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave48_compliance',
  review_run_id: `mrx1000-wave48-compliance-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'compliance',
  expected_hero_sha256: heroSha,
  expected_inline_sha256: inlineSha,
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; hero/share and inline image hashes are separately locked.`,
    'The article states educational and professional boundaries, discloses MRX’s potential economic interest, separates a directional asset range, appraisal conclusion, written proposal, and expected owner net, labels observed, reported, assumed, inferred, and professionally concluded inputs, and routes unresolved title, legal, tax, accounting, appraisal, land, engineering, geology, reserves, commodity, and transaction questions without making owner-specific conclusions.',
...
console.log(
  `Built three hash-locked Wave 48 review artifacts with ${sources.length} live source checks.`,
);
