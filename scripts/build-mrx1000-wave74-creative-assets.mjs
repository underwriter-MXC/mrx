...
const inlineFilename = 'mineral-rights-document-redaction-checklist';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave74-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave74-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave74-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Premium photorealistic eye-level three-quarter records-desk scene with an untouched original stack protected in an archival folder, a visually separate duplicate share copy with opaque redaction bars, a single realistic hand, a plain sleeve, and a marker on the right; quiet dark field on the left; no readable words, names, identifiers, signatures, forms, logos, promises, or fake document text.',
  inline:
    'Materially distinct people-free strict overhead checklist workspace with a protected original folder, separate duplicate on a clipboard with abstract blocks and redaction bars, a separate empty grid log, marker, and paper clips above a quiet lower field; no readable words, identifiers, signatures, forms, logos, promises, or fake document text.',
};
...
function heroTypography() {
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect x="0" y="0" width="730" height="630" fill="url(#navy)" />
      <rect class="rule" x="46" y="102" width="82" height="5" rx="2.5" />
      <text class="title" x="46" y="168">
        <tspan x="46" dy="0">Mineral Rights</tspan>
        <tspan x="46" dy="55">Document Redaction</tspan>
        <tspan x="46" dy="55">Checklist Before You</tspan>
        <tspan x="46" dy="55">Share Records</tspan>
      </text>
    </svg>
  `);
}
...
function inlineTypography() {
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="keyword" x="600" y="536">
        <tspan x="600" dy="0">mineral rights document</tspan>
        <tspan x="600" dy="54">redaction checklist</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave74-ocr-'));
  try {
