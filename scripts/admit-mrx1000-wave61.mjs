...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 61 admission argument(s): ${unknownArgs.join(', ')}`);
}

const slug = 'should-i-sell-my-mineral-rights';
const title = 'Should I Sell My Mineral Rights?';
const primaryKeyword = 'should I sell my mineral rights';
const keyword = 'Sell Mineral Rights or Keep Receiving Royalties';
const heroAlt =
  'A mineral owner compares a royalty statement and a sale folder beside “Should I Sell My Mineral Rights?”.';
const inlineAlt =
  'A parcel map separates a sale folder from royalty statements above “Sell Mineral Rights or Keep Receiving Royalties”.';
const programRowId = 'MRX1000-0141';
const selectionRank = 141;
...
const decisionRelativePath = 'docs/governance/mrx1000-wave61-selection-decision-2026-08-14.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave61-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave61_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W61-SELECT-2026-08-14';
if (
  ...
  throw new Error('Wave 61 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave61' ||
  nestedScalar(fm, 'hero_image', 'src') !== `/assets/articles/hero/${slug}.webp` ||
  nestedScalar(fm, 'hero_image', 'social_src') !== `/assets/articles/hero/${slug}.webp` ||
...
if (
  ...
  creative.hero.public_path !== `/assets/articles/hero/${slug}.webp` ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave61-distinct-generated-sell-or-royalties-decision',
...
if (
  ...
  ledger.articles.some(
    (row) => row.canonical_slug === 'what-every-mineral-rights-owner-needs-to-know',
  )
) {
  throw new Error('Wave 61 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
...
const wave61Rekey = ledger.identity_registry.wave61_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: 'what-every-mineral-rights-owner-needs-to-know',
  prior_searchatlas_handle:
    'searchatlas-topical-map:255581:title:d683f3f7-0aa4-49fa-ac96-e1b78d6dc040',
...
if (wave61Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 61 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave61_rekey = wave61Rekey;
wave61Rekey.draft_selection_decision_sha256 ??= wave61Rekey.selection_decision_sha256;
wave61Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave61Rekey.release_candidate_article_sha256 = articleSha;
wave61Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The broad beginner identity was rejected because live Article122 already owns the exact primary keyword Mineral Rights Explained for Beginners and the new-owner orientation job. The admitted article owns a narrower sell-versus-keep decision record that compares the rights transferred, continuing royalty evidence, timing, uncertainty, liquidity purpose, alternatives, and professional-review questions without deciding for the owner.',
    'Continuous quality-gated admission under D-2026-0804-16 and MRX1000-W61-SELECT-2026-08-14; no numerical cap or elapsed-time gate applies.',
...
batch.decision_authority.wave61_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave61_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave61_selection_decision_sha256 = sha256(decisionBytes);
