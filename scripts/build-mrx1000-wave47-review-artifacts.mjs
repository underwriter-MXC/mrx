...
const slug = 'understanding-texas-mineral-rights-transactions-today';
const programRowId = 'MRX1000-0127';
const title = 'Understanding Texas Mineral Rights Transactions Today';
const inlineKeyword = 'Navigating Mineral Rights Transactions Texas';
const articlePath = `src/content/posts/${slug}.mdx`;
const creativePath = `artifacts/mrx1000-wave47-creative-qa/${slug}/creative-manifest.json`;
...
if (!row || row.slug !== slug || row.title !== title) {
  throw new Error('Wave 47 batch identity is missing or drifted');
}
...
function sourceScope(source) {
  const scopes = new Map([
    ...
    [
      ...
      [
        'The Railroad Commission of Texas provides online query routes for production, drilling permits, well records, organization records, GIS, imaged records, and other operational data, while expressly describing the continually updated datasets as informational, non-authoritative, and without legal force.',
        'The article uses the query menu only to route operational research and preserve the Commission’s stated limitations; it does not treat a query as proof of ownership, title, acreage, payment, contract effect, authority, value, or a transaction result.',
      ],
    ],
    [
      'https://www.rrc.texas.gov/about-us/faqs/oil-gas-faq/production-data-query-system-faqs/',
      [
        'The Railroad Commission PDQ FAQ explains operator-reported production scope, lease-level reporting for oil, reporting lag, later revisions, historical completeness, update frequency, and identifiers used to navigate production queries.',
        'The article uses the FAQ only to explain why source identifiers, scope, periods, access dates, and later corrections must be reconciled; it does not conclude that a public query proves an owner’s payment decimal, royalty amount, title, value, or contract rights.',
      ],
    ],
    [
      'https://www.rrc.texas.gov/about-us/faqs/royalties-faq/',
      [
        'The Railroad Commission royalties FAQ separates county lease or royalty records and Commission operational records from private royalty, lease, payment, and legal matters outside the Commission’s authority, and notes that production identifiers may differ from payment-document identifiers.',
        'The article uses the FAQ only to route county, operational, identifier, and professional questions to the correct lane; it does not interpret a lease, calculate a royalty, determine ownership, or state an owner-specific remedy.',
      ],
    ],
  ]);
...
if (
  ...
  throw new Error(
    'Wave 47 review inputs do not satisfy identity, article-depth, source, or creative gates',
  );
}
...
writeArtifact('editorial', `${programRowId}-${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave47_editorial',
  review_run_id: `mrx1000-wave47-editorial-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'editorial',
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; the answer-first article contains ${wordCount} body tokens and five FAQs.`,
    'The article owns a Texas transaction-source routing job: ownership and authority, asset and operating identity, payment history, directional economics, counterparty and proposal evidence, and document-closing-post-closing records. Every lane records the source, identifier, as-of date, supported question, limitation, conflict status, and professional handoff while routing sequential selling and stage-gate jobs to existing pages.',
...
writeArtifact('factual_citation', `${slug}.review.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave47_factual',
  review_run_id: `mrx1000-wave47-factual-${reviewedAt.replace(/[-:]/g, '')}`,
  ...
  findings: [
    ...
    'MRX claims remain bounded to its current FAQ, methodology, How It Works page, and terms; Railroad Commission query, production, and royalty guidance stays within the agency’s stated operational and informational boundaries; Texas business and recording sources are expressly limited; and IRS claims remain general disposition concepts.',
...
writeArtifact('compliance', `${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave47_compliance',
  review_run_id: `mrx1000-wave47-compliance-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'compliance',
  expected_hero_sha256: heroSha,
  expected_inline_sha256: inlineSha,
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; hero/share and inline image hashes are separately locked.`,
    'The article states educational and professional boundaries, discloses MRX’s potential economic interest, separates county, regulatory, payor, value, proposal, entity, professional, closing, recording, and tax evidence, labels MRX review as directional, and routes unresolved title, legal, tax, accounting, appraisal, land, engineering, cybersecurity, authority, escrow, closing, and transaction questions without making owner-specific conclusions.',
...
console.log(
  `Built three hash-locked Wave 47 review artifacts with ${sources.length} live source checks.`,
);
