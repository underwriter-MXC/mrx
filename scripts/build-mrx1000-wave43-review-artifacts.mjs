...
const slug = 'understanding-mineral-rights-how-to-validate-your-asset-s-true-worth';
const programRowId = 'MRX1000-0123';
const title = 'Understanding Mineral Rights: How to Validate Your Asset’s True Worth';
const inlineKeyword = "How Do I Ensure I'm Receiving a Fair Valuation for My Mineral Rights?";
const articlePath = `src/content/posts/${slug}.mdx`;
const creativePath = `artifacts/mrx1000-wave43-creative-qa/${slug}/creative-manifest.json`;
...
if (!row || row.slug !== slug || row.title !== title) {
  throw new Error('Wave 43 batch identity is missing or drifted');
}
...
function sourceScope(source) {
  const scopes = new Map([
    ...
    [
      'https://www.eia.gov/totalenergy/data/monthly/whatsnew.php',
      [
        'The U.S. Energy Information Administration identifies revised, corrected, estimated, preliminary, and forecast data in its Monthly Energy Review change log.',
        'The article uses the change log only to support recording an evidence cutoff and source-data vintage; it does not infer a property-specific value, forecast, production result, or transaction conclusion.',
      ],
    ],
    [
      'https://www.eia.gov/dnav/pet/pet_pri_spt_s1_d.htm',
      [
        'The U.S. Energy Information Administration publishes dated crude-oil and petroleum-product spot-price series with identified products, periods, and units.',
        'The article uses the page only as public historical price context and requires the exact series, period, unit, access date, and any adjustment to be stated; it does not treat a public price as a property-specific realized price or future guarantee.',
      ],
    ],
    [
      'https://www.eia.gov/dnav/ng/ng_pri_sum_dcu_stx_m.htm',
      [
        'The U.S. Energy Information Administration publishes dated Texas natural-gas price series with identified categories, periods, units, and notes.',
        'The article uses the page only as public historical price context and requires the exact series, period, unit, access date, and any adjustment to be stated; it does not treat a public price as a property-specific realized price or future guarantee.',
      ],
    ],
  ]);
...
if (
  ...
  throw new Error(
    'Wave 43 review inputs do not satisfy identity, article-depth, source, or creative gates',
  );
}
...
writeArtifact('editorial', `${programRowId}-${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave43_editorial',
  review_run_id: `mrx1000-wave43-editorial-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'editorial',
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; the answer-first article contains ${wordCount} body tokens and five FAQs.`,
    'The article owns the valuation-validation and reperformance job: it locks asset scope and evidence dates, traces inputs and transformations, independently reproduces calculations, isolates sensitivities, records variances, and keeps the asset range separate from transaction economics.',
...
writeArtifact('factual_citation', `${slug}.review.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave43_factual',
  review_run_id: `mrx1000-wave43-factual-${reviewedAt.replace(/[-:]/g, '')}`,
  ...
  findings: [
    ...
    'MRX claims remain bounded to its current methodology and FAQ; operational-data claims preserve Railroad Commission reporting scope, lag, and revision limitations; and data-vintage and price-context claims remain bounded to the identified EIA series and change log.',
...
writeArtifact('compliance', `${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave43_compliance',
  review_run_id: `mrx1000-wave43-compliance-${reviewedAt.replace(/[-:]/g, '')}`,
...
console.log(
  `Built three hash-locked Wave 43 review artifacts with ${sources.length} live source checks.`,
);
