...
const keyword = 'Texas Mineral Tax Value vs. Sale Estimate';
const inlineFilename = 'texas-mineral-tax-value-vs-sale-estimate';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave60-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave60-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave60-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Calm documentary Texas mineral-record comparison workspace with a generic county-style tax account folder and a separate voluntary sale-estimate folder aligned by neutral map, well, and evidence tabs beside a deep-navy exact-title field; no official seals, fake forms, extra readable words, account numbers, dollar figures, dates, signatures, private information, logos, appraisal conclusions, tax results, title conclusions, offers, guarantees, buyer claims, or success claims.',
  inline:
    'Compositionally distinct overhead matched-comparison scene with two unlabeled evidence lanes for scope, date, data, assumptions, and exclusions converging on a centered navy exact-keyword card; no official seals, fake forms, extra readable words, account numbers, dollar figures, dates, signatures, private information, logos, appraisal conclusions, tax results, title conclusions, offers, guarantees, buyer claims, or success claims.',
};
...
function heroTypography() {
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <style>
        .title { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 36px; font-weight: 700; letter-spacing: -0.4px; }
        .rule { fill: #d79a2b; }
      </style>
      <rect x="0" y="0" width="660" height="630" fill="url(#navy)" />
      <rect class="rule" x="46" y="95" width="82" height="5" rx="2.5" />
      <text class="title" x="46" y="161">
        <tspan x="46" dy="0">Why Doesn’t My Texas</tspan>
        <tspan x="46" dy="53">Mineral Tax Value Match</tspan>
        <tspan x="46" dy="53">a Sale Estimate?</tspan>
      </text>
    </svg>
  `);
}
...
function inlineTypography() {
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <style>
        .keyword { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 44px; font-weight: 700; text-anchor: middle; letter-spacing: -0.25px; }
        .rule { fill: #d79a2b; }
      </style>
      <rect x="180" y="440" width="840" height="235" fill="url(#navy)" />
      <rect class="rule" x="365" y="473" width="470" height="4" rx="2" />
      <text class="keyword" x="600" y="548">
        <tspan x="600" dy="0">Texas Mineral Tax Value</tspan>
        <tspan x="600" dy="62">vs. Sale Estimate</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave60-ocr-'));
  try {
