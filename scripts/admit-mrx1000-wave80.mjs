...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 80 admission argument(s): ${unknownArgs.join(', ')}`);
}
...
const decisionRelativePath = 'docs/governance/mrx1000-wave80-selection-decision-2026-08-23.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave80-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave80_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W80-SELECT-2026-08-23';
if (
  ...
  throw new Error('Wave 80 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave80' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave80-distinct-generated-overhead-terminology-register-worksheet',
...
const priorCanonicalTitle = 'PDP, PUD, and Undeveloped Acreage in Mineral Valuation';
const priorCanonicalSlug = 'pdp-pud-and-undeveloped-acreage-in-mineral-valuation';
if (
  !sourceRow ||
...
if ([priorCanonicalSlug, slug].includes(sourceRow.canonical_slug)) {
  Object.assign(sourceRow, {
    ...
    secondary_keywords: [keyword, 'oil and gas terminology register', 'reserve report source log'],
    ...
    action_reason:
      'The original PDP/PUD/undeveloped-acreage valuation identity overlapped the live DCF, valuation-methodology, acreage-contiguity, and due-diligence corpus and blurred distinct reserve, acreage, production, and economic concepts. The approved replacement owns only source-preserving capture of displayed terminology, date, preparer, stated standard and edition, and stated scope. It uses only term displayed, standard stated, or standard not stated and stops before classification, compliance, comparability, property attribution, development, production, DCF, offer, or value conclusions. Release remains controlled by the signed batch, matching evidence, deployment, and live verification.',
    ...
    dedupe_evidence: {
      ...
      nearest_same_cluster_slug: 'existing-wells-vs-future-locations-in-a-dcf-model',
      nearest_same_cluster_title_token_jaccard: 0.2308,
      review_status: 'exact_and_fuzzy_title_check_pass',
    },
    exact_slug_unique: true,
    exact_title_unique: true,
    nearest_same_cluster_slug: 'existing-wells-vs-future-locations-in-a-dcf-model',
    cannibalization_score: 0.2308,
...
if (
  ...
  throw new Error('Wave 80 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
const articleSha = sha256(articleBytes);
ledger.identity_registry ??= {};
const wave80Rekey = ledger.identity_registry.wave80_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: priorCanonicalSlug,
  prior_source_handle:
    'factory-taxonomy-synthesis:valuation:pdp-pud-undeveloped-acreage-valuation',
...
if (wave80Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 80 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave80_rekey = wave80Rekey;
wave80Rekey.draft_selection_decision_sha256 ??= wave80Rekey.selection_decision_sha256;
wave80Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave80Rekey.release_candidate_article_sha256 = articleSha;
wave80Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The original PDP/PUD/undeveloped-acreage valuation identity was rejected because it overlaps the live DCF, valuation-methodology, acreage-contiguity, and due-diligence corpus. The admitted replacement owns only a source-preserving terminology register across identified reports, disclosures, and owner documents.',
    'Current official GovInfo, SEC, SPE, and RRC sources support only bounded descriptions of named terminology frameworks, disclosure context, and production-query limitations. None classifies an owner property, certifies or compares a report, establishes compliance, predicts development, supplies DCF inputs, or determines an offer or value.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W80-SELECT-2026-08-23; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 80 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave80_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave80_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave80_selection_decision_sha256 = sha256(decisionBytes);
