...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 77 admission argument(s): ${unknownArgs.join(', ')}`);
}
...
const keyword = 'mineral rights worksheet question locator';
const heroFilename = 'how-to-locate-an-unanswered-question-in-a-mineral-rights-worksheet';
const heroAlt =
...
const decisionRelativePath = 'docs/governance/mrx1000-wave77-selection-decision-2026-08-22.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave77-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave77_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W77-SELECT-2026-08-22';
if (
  ...
  throw new Error('Wave 77 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave77' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave77-distinct-generated-overhead-question-locator-board',
...
const priorCanonicalTitle = 'Net Revenue Interest as a Mineral Valuation Input';
const priorCanonicalSlug = 'net-revenue-interest-as-a-mineral-valuation-input';
if (
  !sourceRow ||
...
if ([priorCanonicalSlug, slug].includes(sourceRow.canonical_slug)) {
  Object.assign(sourceRow, {
    ...
    action_reason:
      'The original NRI-as-value-input identity materially collided with the live net-royalty-acre/NRI and lease-royalty-rate valuation guides. The approved replacement owns only a neutral locator row for one unanswered question in an existing worksheet: displayed file identity, visible location, displayed source reference if any, neutral question, and already-shown handoff recipient. It cannot assert a discrepancy, request a correction, validate a source, alter an input, calculate or certify value, analyze an offer, or recommend a transaction. Release remains controlled by the signed batch, matching evidence, deployment, and live verification.',
    ...
    dedupe_evidence: {
      ...
      nearest_same_cluster_slug: 'mineral-rights-valuation-input-correction-packet',
      nearest_same_cluster_title_token_jaccard: 0.2857,
      review_status: 'exact_and_fuzzy_title_check_pass',
    },
    exact_slug_unique: true,
    exact_title_unique: true,
    nearest_same_cluster_slug: 'mineral-rights-valuation-input-correction-packet',
    cannibalization_score: 0.2857,
...
if (
  ...
  throw new Error('Wave 77 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
const articleSha = sha256(articleBytes);
ledger.identity_registry ??= {};
const wave77Rekey = ledger.identity_registry.wave77_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: priorCanonicalSlug,
  prior_source_handle:
    'factory-taxonomy-synthesis:valuation:net-revenue-interest-mineral-valuation',
...
if (wave77Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 77 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave77_rekey = wave77Rekey;
wave77Rekey.draft_selection_decision_sha256 ??= wave77Rekey.selection_decision_sha256;
wave77Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave77Rekey.release_candidate_article_sha256 = articleSha;
wave77Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The original NRI-as-value-input identity was rejected because it overlaps the live net-royalty-acre/NRI and lease-royalty-rate valuation guides. The admitted replacement owns only the neutral mechanics for locating one unanswered question in an existing worksheet and routing it to an already-shown reviewer or queue.',
    'NARA, Microsoft, and RRC sources support only bounded descriptions of record identifiers and dates, worksheet location references, query types and update context, and PDQ navigation/reporting limitations. None supplies a source-validation judgment, correction, ownership fact, title conclusion, appraisal, value, offer analysis, or transaction outcome.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W77-SELECT-2026-08-22; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 77 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave77_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave77_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave77_selection_decision_sha256 = sha256(decisionBytes);
