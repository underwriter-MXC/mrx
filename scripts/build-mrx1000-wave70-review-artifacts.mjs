...
const slug = 'mineral-rights-valuation-input-correction-packet';
const programRowId = 'MRX1000-0150';
const title = 'How to Build a Mineral Rights Valuation Input Correction Packet';
const primaryKeyword = 'mineral rights valuation input correction packet';
const inlineKeyword = 'Mineral Rights Valuation Input Correction Packet';
const heroAlt =
  'An owner reviews blank correction cards beside “How to Build a Mineral Rights Valuation Input Correction Packet”.';
const inlineAlt =
  'Blank correction fields and source markers appear above “Mineral Rights Valuation Input Correction Packet”.';
const articlePath = `src/content/posts/${slug}.mdx`;
const creativePath = `artifacts/mrx1000-wave70-creative-qa/${slug}/creative-manifest.json`;
...
const sourceReceiptPath = process.env.MRX_WAVE70_SOURCE_RECEIPTS_PATH ?? null;
...
if (!row || row.slug !== slug || row.title !== title || row.selection_rank !== 150) {
  throw new Error('Wave 70 batch identity is missing or drifted');
}
...
async function verifySource({ label, url }) {
  if (sourceReceiptBundle) {
    ...
    if (!Number.isFinite(checkedAt) || ageMs < -300_000 || ageMs > 7_200_000) {
      throw new Error('Wave 70 preverified source receipts are invalid, future-dated, or stale');
    }
...
const sourceScopes = new Map([
  [
    ...
    [
      'The current EPA page identifies its Information Quality Guidelines and the federal correction-request mechanism for information disseminated by EPA.',
      'The article identifies the EPA process as a federal administrative analogy only. It does not claim that the process governs a private mineral-rights valuation, validates a replacement value, or creates a right to a revised result.',
    ],
  ],
  [
    'https://www.epa.gov/sites/production/files/2020-02/documents/epa-info-quality-guidelines_pdf_version.pdf',
    [
      'Section 8.2 of the EPA guideline PDF describes correction requests that identify the requester, cite the disputed information, explain the concern, recommend corrective action, and describe the requester’s interest.',
      'The article borrows only that field-level request discipline. It omits agency-specific legal standards and makes no private right, validation, value, or correction conclusion.',
    ],
  ],
  [
    'https://www.archives.gov/records-mgmt/scheduling/basics',
    [
      'The NARA Records Basics page distinguishes source records, input records, system content, and outputs in federal records-management guidance.',
      'The article uses that distinction only as a transparent administrative analogy for preserving the reviewed snapshot, source, input, and later output separately. It does not impose a private retention rule.',
    ],
  ],
  [
    'https://mineralrightsxchange.com/blog/understanding-mineral-rights-how-to-validate-your-asset-s-true-worth/',
    [
      'The current MRX valuation-validation guide owns evidence review, transformation tracing, method and assumption review, reperformance, and result-difference analysis.',
      'The correction-packet article begins only after a specific discrepancy has already been identified. It does not validate evidence, reperform a valuation, run sensitivities, or decide whether a requested replacement is correct.',
    ],
  ],
  [
    'https://mineralrightsxchange.com/blog/understanding-texas-mineral-rights-valuation-process/',
    [
      'The current MRX Texas valuation-process guide owns the traceable relationship among the analyzed interest, evidence, inputs, assumptions, range, and unresolved questions.',
      'The article uses that guide only to define the adjacent traceability job. It does not explain valuation methods, calculate value, revise a range, appraise an interest, or decide offer fairness.',
    ],
  ],
  [
    'https://mineralrightsxchange.com/blog/how-to-build-a-mineral-rights-sale-document-package-index/',
    [
      'The current MRX document-package guide owns received-file identity, page and exhibit completeness, versions, and controlled component references.',
      'The correction packet links to a stable component and page rather than rebuilding the package. It does not determine authenticity, completeness, title, legal effect, acreage, decimal interest, or document sufficiency.',
    ],
  ],
]);
...
if (
  ...
  throw new Error(
    'Wave 70 review inputs do not satisfy identity, article-depth, source, or creative gates',
  );
}
...
writeArtifact('editorial', `${programRowId}-${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave70_editorial',
  review_run_id: `mrx1000-wave70-editorial-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'editorial',
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; the answer-first article contains ${wordCount} body tokens and five FAQs.`,
    'The article owns a neutral correction-intake job only after a specific factual input discrepancy has already been identified. It records the field, observed value, source location, requested replacement, stated reason, support reference, and one of six controlled workflow statuses while excluding validation, reperformance, recalculation, value, title, acreage, decimal, fairness, appraisal, and sell-or-hold conclusions.',
...
writeArtifact('factual_citation', `${slug}.review.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave70_factual',
  review_run_id: `mrx1000-wave70-factual-${reviewedAt.replace(/[-:]/g, '')}`,
  ...
  findings: [
    ...
    'Claims remain bounded to the EPA correction-request fields and NARA source/input/output distinction as explicitly nonbinding federal administrative analogies, plus current MRX validation, valuation-process, and package-index boundaries. No source is converted into a private correction right, source validation, title, acreage, decimal, value, fairness, appraisal, contract, or transaction conclusion.',
    'The article invents no real owner, request, input, source record, property, buyer, contract, transaction, price, valuation result, testimonial, success rate, or owner-specific legal, tax, accounting, title, appraisal, investment, or transaction conclusion.',
  ],
  checks: [
...
writeArtifact('compliance', `${slug}.json`, {
  ...
  ...common,
  reviewer_id: 'codex_wave70_compliance',
  review_run_id: `mrx1000-wave70-compliance-${reviewedAt.replace(/[-:]/g, '')}`,
  capability: 'compliance',
  expected_hero_sha256: heroSha,
  expected_inline_sha256: inlineSha,
  findings: [
    `Complete current MDX SHA-256 is ${articleSha}; hero/share and inline image hashes are separately locked.`,
    'The article states educational and professional boundaries, separates correction intake from validation, valuation revision, package completeness, title, acreage, appraisal, and decisions, discloses possible MRX buyer interest, preserves owner agency, and makes no owner-specific correction, ownership, title, authority, value, fairness, buyer, contract, legal, tax, suitability, investment, or transaction conclusion.',
    'Image text is limited to the exact article title and approved correction-packet phrase and adds no request content, name, address, account data, property identifier, amount, acreage, decimal, date, signature, official seal, legal conclusion, value conclusion, ranking, recommendation, offer, guarantee, buyer endorsement, or transaction result.',
  ],
  checks: [
    'complete_file_sha256_match',
...
console.log(
  `Built three hash-locked Wave 70 review artifacts with ${sources.length} current source checks.`,
);
