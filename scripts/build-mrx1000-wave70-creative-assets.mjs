...
const inlineFilename = 'mineral-rights-valuation-input-correction-packet';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave70-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave70-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave70-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Premium documentary photograph of one adult mineral owner at an eye-level three-quarter desk view on the right, calmly reviewing blank correction cards beside a closed navy source folder and a neutral page marker, with warm natural window light and a clean deep-navy exact-title field on the left; no readable records, names, addresses, dates, prices, acreage, decimals, signatures, official seals, buyer branding, fake evidence, legal conclusions, value conclusions, rankings, recommendations, guarantees, or success claims.',
  inline:
    'Compositionally distinct people-free overhead valuation-input correction workspace with a blank field grid, closed navy source folders, a neutral page marker, paper clips, magnifier, ruler, pencil, and calculator above a clean deep-navy lower exact-phrase field; no people, hands, screens, readable records, names, addresses, dates, prices, acreage, decimals, signatures, official seals, buyer branding, fake evidence, legal conclusions, value conclusions, rankings, recommendations, guarantees, or success claims.',
};
...
function heroTypography() {
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <style>
        .title { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 38px; font-weight: 700; letter-spacing: -0.35px; }
        .rule { fill: #d79a2b; }
      </style>
      <rect x="0" y="0" width="700" height="630" fill="url(#navy)" />
      <rect class="rule" x="46" y="116" width="82" height="5" rx="2.5" />
      <text class="title" x="46" y="187">
        <tspan x="46" dy="0">How to Build a</tspan>
        <tspan x="46" dy="58">Mineral Rights</tspan>
        <tspan x="46" dy="58">Valuation Input</tspan>
        <tspan x="46" dy="58">Correction Packet</tspan>
      </text>
    </svg>
  `);
}
...
function inlineTypography() {
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <style>
        .keyword { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 40px; font-weight: 700; text-anchor: middle; letter-spacing: -0.25px; }
        .rule { fill: #d79a2b; }
      </style>
      ...
      <text class="keyword" x="600" y="530">
        <tspan x="600" dy="0">Mineral Rights Valuation</tspan>
        <tspan x="600" dy="54">Input Correction Packet</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave70-ocr-'));
  try {
