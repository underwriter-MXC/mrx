...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 65 admission argument(s): ${unknownArgs.join(', ')}`);
}
...
const keyword = 'Mineral Rights Sale Package Index';
const heroFilename = 'how-to-build-a-mineral-rights-sale-document-package-index';
const heroAlt =
...
const decisionRelativePath = 'docs/governance/mrx1000-wave65-selection-decision-2026-08-14.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave65-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave65_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W65-SELECT-2026-08-14';
if (
  ...
  throw new Error('Wave 65 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave65' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave65-distinct-generated-overhead-document-package',
...
const priorCanonicalTitle = 'What to Avoid in the Mineral Rights Selling Process';
const priorCanonicalSlug = 'what-to-avoid-in-the-mineral-rights-selling-process';
if (
  !sourceRow ||
...
if (sourceRow.canonical_slug === priorCanonicalSlug) {
  ...
  action_reason:
    'The replacement document-package-index article was selected after exact-keyword and content-job collision rejection and remains controlled by the signed batch, matching evidence, deployment, and live verification.',
  ...
  dedupe_evidence: {
    ...
    nearest_same_cluster_slug: 'mineral-rights-sale-decision-log',
...
if (
  ...
  throw new Error('Wave 65 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
const articleSha = sha256(articleBytes);
ledger.identity_registry ??= {};
const wave65Rekey = ledger.identity_registry.wave65_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: priorCanonicalSlug,
  prior_searchatlas_handle:
    'searchatlas-topical-map:256286:title:25fb4b80-2420-487e-8f7c-64eafede5cbe',
...
if (wave65Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 65 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave65_rekey = wave65Rekey;
wave65Rekey.draft_selection_decision_sha256 ??= wave65Rekey.selection_decision_sha256;
wave65Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave65Rekey.release_candidate_article_sha256 = articleSha;
wave65Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The original seller-process-errors identity was rejected because a live article owns the exact primary keyword and several live articles own the mistakes-and-avoidance job. The admitted replacement owns only a page-level completeness index for a mineral-rights sale document package.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W65-SELECT-2026-08-14; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 65 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave65_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave65_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave65_selection_decision_sha256 = sha256(decisionBytes);
