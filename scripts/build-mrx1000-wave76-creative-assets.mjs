...
const inlineFilename = 'mineral-valuation-sensitivity-analysis-worksheet';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave76-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave76-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave76-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Premium photorealistic side-angle scene with one calm reviewer and five completely blank scenario cards on the right; one card is shifted and marked only by a gold border while the other four remain fixed; quiet dark field on the left; no maps, values, charts, readable words, names, identifiers, signatures, forms, logos, promises, or fake document text.',
  inline:
    'Materially distinct people-free strict overhead worksheet grid with one highlighted row, two blank input cells, adjacent fixed blank fields, a blank source-date tab, and a blank output field above a quiet lower field; no values, charts, readable words, identifiers, signatures, forms, logos, promises, or fake document text.',
};
  ...
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="title" x="46" y="168">
        <tspan x="46" dy="0">Mineral Valuation</tspan>
        <tspan x="46" dy="51">Sensitivity Analysis:</tspan>
        <tspan x="46" dy="51">A One-Variable</tspan>
        <tspan x="46" dy="51">Scenario Worksheet</tspan>
      </text>
    </svg>
  `);
}
...
function inlineTypography() {
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <style>
        .keyword { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 39px; font-weight: 700; text-anchor: middle; letter-spacing: -0.25px; }
        .rule { fill: #d79a2b; }
      </style>
      ...
      <text class="keyword" x="600" y="536">
        <tspan x="600" dy="0">mineral valuation sensitivity</tspan>
        <tspan x="600" dy="54">analysis worksheet</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave76-ocr-'));
  try {
