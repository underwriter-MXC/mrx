...
const inlineFilename = 'mineral-rights-offer-sender-identity-cross-check';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave72-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave72-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave72-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Photorealistic eye-level office scene with a mineral owner and analyst entirely on the right, reviewing a generic blank offer and sender-detail checklist, with a quiet neutral title field on the left; no readable records, names, addresses, dates, prices, acreage, signatures, seals, buyer branding, identity result, legitimacy conclusion, ranking, recommendation, guarantee, or success claim.',
  inline:
    'Compositionally distinct people-free strict top-down sender cross-check workspace with three blank evidence cards, a blank seven-row comparison matrix, calculator, and unlabeled tract-outline sheet above a clean deep-navy lower exact-keyword field; no people, hands, readable records, names, addresses, dates, prices, signatures, seals, buyer branding, match or mismatch result, legitimacy conclusion, ranking, recommendation, guarantee, or success claim.',
};
...
function heroTypography() {
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <style>
        .title { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 36px; font-weight: 700; letter-spacing: -0.35px; }
        .rule { fill: #d79a2b; }
      </style>
      ...
      <text class="title" x="46" y="187">
        ...
        <tspan x="46" dy="58">Offer Sender Identity</tspan>
        <tspan x="46" dy="58">Cross-Check</tspan>
      </text>
    </svg>
  `);
}
  ...
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="keyword" x="600" y="530">
        <tspan x="600" dy="0">mineral rights offer sender</tspan>
        <tspan x="600" dy="54">identity cross-check</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave72-ocr-'));
  try {
