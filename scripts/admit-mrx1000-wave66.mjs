...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 66 admission argument(s): ${unknownArgs.join(', ')}`);
}
...
const heroFilename = 'how-to-talk-to-your-family-before-you-decide-to-sell-mineral-rights';
const heroAlt =
...
const decisionRelativePath = 'docs/governance/mrx1000-wave66-selection-decision-2026-08-14.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave66-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave66_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W66-SELECT-2026-08-14';
if (
  ...
  throw new Error('Wave 66 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave66' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave66-distinct-generated-overhead-family-conversation-workspace',
...
if (sourceRow.canonical_slug === priorCanonicalSlug) {
  ...
  action_reason:
    'The replacement family-conversation article was selected after the assessment-expectations identity collided with live assessment, booking, preparation, service-scope, and seller-process jobs. It remains controlled by the signed batch, matching evidence, deployment, and live verification.',
  ...
  dedupe_evidence: {
    ...
    nearest_same_cluster_slug: 'mineral-rights-inheritance-in-texas-what-heirs-need-to-know-before-selling',
    nearest_same_cluster_title_token_jaccard: 0.3846,
    review_status: 'exact_and_fuzzy_title_check_pass',
  },
  exact_slug_unique: true,
  exact_title_unique: true,
  nearest_same_cluster_slug: 'mineral-rights-inheritance-in-texas-what-heirs-need-to-know-before-selling',
  cannibalization_score: 0.3846,
...
if (
  ...
  throw new Error('Wave 66 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
const articleSha = sha256(articleBytes);
ledger.identity_registry ??= {};
const wave66Rekey = ledger.identity_registry.wave66_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: priorCanonicalSlug,
  prior_searchatlas_handle:
    'searchatlas-topical-map:256286:title:c9368b45-a38d-4833-bfdb-cbeda2fb1be5',
...
if (wave66Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 66 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave66_rekey = wave66Rekey;
wave66Rekey.draft_selection_decision_sha256 ??= wave66Rekey.selection_decision_sha256;
wave66Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave66Rekey.release_candidate_article_sha256 = articleSha;
wave66Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The original no-obligation-assessment-process identity was rejected because live articles already own assessment expectations, booking, consultation preparation, service scope, and seller-process sequence. The admitted replacement owns only the internal family conversation before a possible assessment.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W66-SELECT-2026-08-14; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 66 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave66_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave66_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave66_selection_decision_sha256 = sha256(decisionBytes);
