...
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave26-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave26-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave26-creative-qa/${slug}`),
};

const generationPrompts = {
  hero:
    'Photorealistic mineral-owner evidence workspace with a mapped tract, abstract production curves, royalty-statement forms, a checklist, human context at the edge, a dark exact-title panel, and no readable words, letters, or numbers outside the exact article title.',
  inline:
    'Compositionally distinct overhead five-part evidence board with ownership, payment, production, lease, and uncertainty cues, an exact-keyword navy band, and no readable words, letters, or numbers outside the exact keyword.',
};
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave26-ocr-'));
  try {
