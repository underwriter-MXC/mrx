...
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave34-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave34-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave34-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Photorealistic mineral owner calmly reviewing mineral-rights evidence with a professional while retaining physical control of the documents, with a dark title-safe panel and no readable words, letters, numbers, prices, percentages, formulas, signatures, logos, or seals.',
  inline:
    'Compositionally distinct no-people overhead permission-and-options path with six connected pictogram cards for property, records, production, scenarios, privacy, and an open decision gate, plus a lower keyword-safe field and no readable words, letters, numbers, prices, percentages, formulas, signatures, logos, or seals.',
};
  ...
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="title" x="72" y="171">
        ...
        <tspan x="72" dy="54">Factors Without</tspan>
        <tspan x="72" dy="54">Obligation</tspan>
      </text>
    </svg>
  `);
}
...
function inlineTypography() {
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect class="rule" x="533" y="421" width="134" height="5" rx="2.5" />
      <text class="keyword" x="600" y="493">
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave34-ocr-'));
  try {
