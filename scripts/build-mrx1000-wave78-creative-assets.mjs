...
const inlineFilename = 'offset-activity-property-connection';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave78-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave78-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave78-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Premium photorealistic side-angle mineral-owner desk with a blank regulatory well record, a separate blank deed-and-lease folder, three wordless status tokens, and one restrained pointing hand on the right; quiet navy field on the left; no values, readable words, names, identifiers, signatures, logos, promises, or fake document text.',
  inline:
    'Materially distinct people-free strict overhead pale cross-check matrix with a blank wellbore card, blank owner-record cards, three wordless status markers, a blank map tile, and a blank source tag above a quiet lower navy field; no values, readable words, identifiers, signatures, logos, promises, or fake document text.',
};
  ...
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="title" x="46" y="168">
        <tspan x="46" dy="0">Offset Activity Records:</tspan>
        <tspan x="46" dy="51">Building a</tspan>
        <tspan x="46" dy="51">Property-Connection</tspan>
        <tspan x="46" dy="51">Cross-Check</tspan>
      </text>
    </svg>
  `);
}
  ...
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="keyword" x="600" y="536">
        <tspan x="600" dy="0">offset activity</tspan>
        <tspan x="600" dy="54">property connection</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave78-ocr-'));
  try {
