...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 69 admission argument(s): ${unknownArgs.join(', ')}`);
}
...
const keyword = 'Mineral Rights Offer Correspondence Index';
const heroFilename = 'how-to-build-a-mineral-rights-offer-correspondence-index';
const heroAlt =
...
const decisionRelativePath = 'docs/governance/mrx1000-wave69-selection-decision-2026-08-22.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave69-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave69_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W69-SELECT-2026-08-22';
if (
  ...
  throw new Error('Wave 69 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave69' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave69-distinct-generated-overhead-correspondence-index-workspace',
...
const priorCanonicalTitle = 'Why You Should Avoid These Seller Pitfalls';
const priorCanonicalSlug = 'why-you-should-avoid-these-seller-pitfalls';
if (
  !sourceRow ||
...
if (sourceRow.canonical_slug === priorCanonicalSlug) {
  ...
  action_reason:
    'The original seller-pitfalls identity directly collided with the live seller-mistakes article and nearby mistakes, missteps, traps, offer-comparison, and sale-process pages. The approved replacement owns only a neutral directory of written offer communications and their attachment relationships after intake and package indexing, with outcomes limited to indexed, linked to package, duplicate noted, or not available. It remains controlled by the signed batch, matching evidence, deployment, and live verification.',
  ...
  dedupe_evidence: {
    ...
    nearest_same_cluster_slug: 'how-to-build-a-mineral-rights-sale-document-package-index',
    nearest_same_cluster_title_token_jaccard: 0.5833,
    review_status: 'exact_and_fuzzy_title_check_pass',
  },
  exact_slug_unique: true,
  exact_title_unique: true,
  nearest_same_cluster_slug: 'how-to-build-a-mineral-rights-sale-document-package-index',
  cannibalization_score: 0.5833,
...
if (
  ...
  throw new Error('Wave 69 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
const articleSha = sha256(articleBytes);
ledger.identity_registry ??= {};
const wave69Rekey = ledger.identity_registry.wave69_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: priorCanonicalSlug,
  prior_searchatlas_handle:
    'searchatlas-topical-map:256286:title:3f92931f-23fb-4862-bad6-22db5e92d0ad',
...
if (wave69Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 69 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave69_rekey = wave69Rekey;
wave69Rekey.draft_selection_decision_sha256 ??= wave69Rekey.selection_decision_sha256;
wave69Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave69Rekey.release_candidate_article_sha256 = articleSha;
wave69Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The original seller-pitfalls identity was rejected because the live seller-mistakes article already owns that prevention job and nearby mistakes, missteps, traps, offer-comparison, and sale-process pages make a generic rewrite unsafe. The admitted replacement owns only a neutral correspondence index after intake and package indexing.',
    'NARA sources are used only as a transparent records-management analogy for observable message and attachment fields, not as a private-owner transfer or retention requirement. Texas Property Code Chapter 5 is used only for a bounded document-linking example and never for coverage, compliance, delivery, enforceability, remedy, or voidness conclusions.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W69-SELECT-2026-08-22; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 69 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave69_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave69_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave69_selection_decision_sha256 = sha256(decisionBytes);
