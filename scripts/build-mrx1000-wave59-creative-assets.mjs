...
const keyword = 'Texas Mineral Rights Sale Problem Triage';
const inlineFilename = 'texas-mineral-rights-sale-problem-triage';
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave59-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave59-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave59-creative-qa/${slug}`),
};

const generationPrompts = {
  hero: 'Calm documentary Texas mineral-rights transaction-triage workspace with one preserved file anchored by a last-verified-state marker and distinct unsigned, signed, payment, recording, and post-close stage cues beside a deep-navy exact-title field; no extra readable words, numbers, amounts, dates, signatures, private information, logos, accusations, legal conclusions, title conclusions, fraud findings, remedies, recovery claims, buyer claims, or success claims.',
  inline:
    'Compositionally distinct overhead incident card routing frozen evidence, ordinary discrepancy, security, recording, payor, and tax questions to separate unlabeled destinations around a centered navy exact-keyword card; no extra readable words, numbers, amounts, dates, signatures, private information, logos, accusations, legal conclusions, title conclusions, fraud findings, remedies, recovery claims, buyer claims, or success claims.',
};
...
function heroTypography() {
  return Buffer.from(`
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      ...
      <style>
        .title { fill: #fffaf0; font-family: Georgia, 'Times New Roman', serif; font-size: 40px; font-weight: 700; letter-spacing: -0.45px; }
        .rule { fill: #d79a2b; }
      </style>
      ...
      <text class="title" x="46" y="169">
        <tspan x="46" dy="0">What Should I Do If a Texas</tspan>
        <tspan x="46" dy="58">Mineral Rights Sale</tspan>
        <tspan x="46" dy="58">Goes Wrong?</tspan>
      </text>
    </svg>
  `);
}
...
function inlineTypography() {
  return Buffer.from(`
    <svg width="1200" height="675" xmlns="http://www.w3.org/2000/svg">
      ...
      <text class="keyword" x="600" y="320">
        <tspan x="600" dy="0">Texas Mineral Rights</tspan>
        <tspan x="600" dy="62">Sale Problem Triage</tspan>
      </text>
    </svg>
  `);
}
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave59-ocr-'));
  try {
