...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 76 admission argument(s): ${unknownArgs.join(', ')}`);
}
...
const keyword = 'mineral valuation sensitivity analysis worksheet';
const heroFilename = 'mineral-valuation-sensitivity-analysis-a-one-variable-scenario-worksheet';
const heroAlt =
...
const decisionRelativePath = 'docs/governance/mrx1000-wave76-selection-decision-2026-08-22.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave76-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave76_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W76-SELECT-2026-08-22';
if (
  ...
  throw new Error('Wave 76 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave76' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave76-distinct-generated-overhead-one-variable-scenario-grid',
...
const priorCanonicalTitle = 'Mineral Valuation Sensitivity Analysis: Which Assumptions Matter?';
const priorCanonicalSlug = 'mineral-valuation-sensitivity-analysis-which-assumptions-matter';
if (
  !sourceRow ||
...
if ([priorCanonicalSlug, slug].includes(sourceRow.canonical_slug)) {
  Object.assign(sourceRow, {
    ...
    action_reason:
      'The original broad sensitivity-analysis identity materially collided with live range-width, valuation-factor, price-deck, production-decline, discount-rate, existing-well, and future-location content. The approved replacement owns only a one-variable scenario control worksheet applied to an existing model: one sourced input changes, all other assumptions and conventions remain fixed, and the supplied model output is recorded. It cannot create inputs, rank assumptions, assign probabilities, forecast development, calculate or certify value, analyze an offer, or recommend a transaction. Release remains controlled by the signed batch, matching evidence, deployment, and live verification.',
    ...
    dedupe_evidence: {
      ...
      nearest_same_cluster_slug: 'why-is-my-mineral-rights-valuation-range-so-wide',
      nearest_same_cluster_title_token_jaccard: 0.2963,
      review_status: 'exact_and_fuzzy_title_check_pass',
    },
    exact_slug_unique: true,
    exact_title_unique: true,
    nearest_same_cluster_slug: 'why-is-my-mineral-rights-valuation-range-so-wide',
    cannibalization_score: 0.2963,
...
if (
  ...
  throw new Error('Wave 76 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
const articleSha = sha256(articleBytes);
ledger.identity_registry ??= {};
const wave76Rekey = ledger.identity_registry.wave76_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: priorCanonicalSlug,
  prior_source_handle:
    'factory-taxonomy-synthesis:valuation:mineral-valuation-sensitivity-analysis',
...
if (wave76Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 76 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave76_rekey = wave76Rekey;
wave76Rekey.draft_selection_decision_sha256 ??= wave76Rekey.selection_decision_sha256;
wave76Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave76Rekey.release_candidate_article_sha256 = articleSha;
wave76Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The original broad sensitivity-analysis identity was rejected because it overlapped live range-width, valuation-factor, price-deck, production-decline, discount-rate, existing-well, and future-location material. The admitted replacement owns only the control mechanics for changing one sourced input in an existing worksheet while every other assumption and convention stays fixed.',
    'RRC and EIA sources support only bounded descriptions of dated public production and commodity data plus reporting limitations. None supplies a property-specific model input, public-record match, ownership fact, title conclusion, forecast, probability, appraisal, value, offer analysis, or transaction outcome.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W76-SELECT-2026-08-22; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 76 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave76_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave76_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave76_selection_decision_sha256 = sha256(decisionBytes);
