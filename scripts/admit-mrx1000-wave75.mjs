...
if (unknownArgs.length) {
  throw new Error(`Unknown Wave 75 admission argument(s): ${unknownArgs.join(', ')}`);
}
...
const keyword = 'mineral rights AI source trace';
const heroFilename = 'how-to-keep-an-ai-source-trace-for-a-mineral-rights-review';
const heroAlt =
...
const decisionRelativePath = 'docs/governance/mrx1000-wave75-selection-decision-2026-08-22.md';
const creativeManifestRelativePath = `artifacts/mrx1000-wave75-creative-qa/${slug}/creative-manifest.json`;
...
async function writeReviewPlaceholder(relativePath, capability, now) {
  ...
  const artifact = {
    artifact_type: 'mrx1000_wave75_review_placeholder',
...
const requiredDecisionId = 'MRX1000-W75-SELECT-2026-08-22';
if (
  ...
  throw new Error('Wave 75 selection decision remains draft-only and is not publication authority');
}
...
if (
  ...
  scalar(fm, 'reviewed_by') !== 'mrx_compliance-continuous-wave75' ||
...
const retrofitRow = {
  ...
  inline: {
    ...
    visual_variant: 'wave75-distinct-generated-overhead-ai-source-provenance-grid',
...
if ([priorCanonicalSlug, slug].includes(sourceRow.canonical_slug)) {
  Object.assign(sourceRow, {
    ...
    pillar_url: '/methodology/',
    cluster: 'mrx-methodology-transparency-underwriter-process',
    primary_keyword: primaryKeyword,
    secondary_keywords: [keyword, 'AI source provenance', 'trace AI review statements'],
    ...
    action_reason:
      'The original AI offer-comparison identity materially collided with live platform, offer-comparison, methodology, valuation, and seller-decision content. The approved replacement owns only a statement-level provenance note that labels owner-provided facts, public-source facts, and unresolved inferences, then records source reference, access date, and human-review status. It cannot compare providers or offers, describe MRX differentiation, calculate value or ownership, make a legal or tax conclusion, or promise AI accuracy, completeness, reliability, safety, or reduced risk. Release remains controlled by the signed batch, matching evidence, deployment, and live verification.',
    ...
    dedupe_evidence: {
      ...
      nearest_same_cluster_title_token_jaccard: 0.2381,
...
if (
  ...
  throw new Error('Wave 75 canonical ledger re-key or 1,000-row uniqueness proof failed');
}
const articleSha = sha256(articleBytes);
ledger.identity_registry ??= {};
const wave75Rekey = ledger.identity_registry.wave75_rekey ?? {
  program_row_id: programRowId,
  prior_canonical_slug: priorCanonicalSlug,
  prior_source_handle: 'searchatlas-topical-map:254654:title:89816c5d-85ef-445b-9f04-044650c63561',
...
if (wave75Rekey.program_row_id !== programRowId) {
  throw new Error('Wave 75 canonical re-key provenance is missing or drifted');
}
ledger.identity_registry.wave75_rekey = wave75Rekey;
wave75Rekey.draft_selection_decision_sha256 ??= wave75Rekey.selection_decision_sha256;
wave75Rekey.selection_decision_sha256 = sha256(decisionBytes);
wave75Rekey.release_candidate_article_sha256 = articleSha;
wave75Rekey.promoted_at_utc ??= now;
...
const batchRow = {
  ...
  risk_citation_remediation: [
    'The original identity was rejected because its AI offer-comparison promise overlapped live platform, offer-comparison, methodology, valuation, and seller-decision material. The admitted replacement owns only statement-level provenance labeling with an exact source reference, access date, and human-review status.',
    'MRX, NIST, and FTC sources support only bounded first-party source and privacy disclosures, voluntary risk-management context, and the need for evidence behind specified AI accuracy or efficacy claims. None proves an AI output, source match, owner fact, review result, accuracy, completeness, reliability, safety, title, value, legal conclusion, tax treatment, or transaction outcome.',
    'Continuous quality-gated admission under D-2026-0804-16, the 2026-08-14 no-approval owner directive, and MRX1000-W75-SELECT-2026-08-22; no numerical cap, elapsed-time gate, or owner publication approval applies.',
...
if (
  ...
  throw new Error('Wave 75 continuous batch identity or selection-rank audit failed');
}
...
batch.decision_authority.wave75_selection_decision_id = requiredDecisionId;
batch.decision_authority.wave75_selection_decision_path = decisionRelativePath;
batch.decision_authority.wave75_selection_decision_sha256 = sha256(decisionBytes);
