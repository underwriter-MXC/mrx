...
const paths = {
  heroSource: join(root, `artifacts/mrx1000-wave30-creative-sources/${slug}-hero-base.png`),
  inlineSource: join(root, `artifacts/mrx1000-wave30-creative-sources/${slug}-inline-base.png`),
  ...
  qa: join(root, `artifacts/mrx1000-wave30-creative-qa/${slug}`),
};

const generationPrompts = {
  hero:
    'Photorealistic mineral owner and transaction guide reviewing a controlled sequence of blank folders in a modern ranch office, with a dark exact-title panel and no readable words, letters, or numbers outside the exact article title.',
  inline:
    'Compositionally distinct no-people overhead seller-control handoff map with six blank pictogram cards, brass connectors, a central exact-keyword folder, and no readable words, letters, or numbers outside the exact keyword.',
};
...
async function main() {
  ...
  const tempDirectory = await mkdtemp(join(tmpdir(), 'mrx-wave30-ocr-'));
  try {
