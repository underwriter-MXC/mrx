...
export function buildOgImage(path: string, baseUrl: string, defaultOg = '/og-default.svg'): string {
  // Per SEO plan §1: per-page OG image, defaulting to /og-default.png.
  // We ship an SVG default for the MVP; PNGs come from a later stage.
  if (path === '/') return `${baseUrl}${defaultOg}`;
  // Per-page OG images are generated by scripts/og-card.mjs in a later stage.
  return `${baseUrl}/og${path.replace(/\/$/, '')}.svg`;
}
